...
which.input <- list(
  ...
  which.gene.set =c('CD34', 'CDX2', 'SOX11', 'SOX4', 'ZDHHC17', 'TGFBR2', 'LRP6', 'SMAD1', 'PLA2G12A', 'TADA2B', 'PHB2', 'DOT1L', 'SUPT20H', 'GYPA', 'SPN', 'KDR', 'CDX4', 'CRKL', 'PRKCA', 'CDH1', 'HOXA9', 'AKR1A1') # character (e.g., sex.specific)
)
...
if (nrow(all.markers) > 0){
  all.markers[, c("p_val", "avg_log2FC", "p_val_adj")] <- signif(all.markers[, c("p_val", "avg_log2FC", "p_val_adj")], 3)
...
```{r}
# 
# getExpressionMatrix.dev <- function(so, only.variable = F, which.assay = NULL, which.data = "scale", use.additional.genes = NA, as.dense = F){
# 
#   # specify assay
...
# plt.top.gsea[[available_markers[2]]]
