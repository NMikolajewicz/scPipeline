...
packages2load <- c("scMiko", "Seurat", "plyr",  "dplyr", "tidyr", "reshape2", "gridExtra",
                   ...
                   "AnnotationDbi", "org.Mm.eg.db", "org.Hs.eg.db", "fgsea", "ggplot2", "reactome.db", "schex")
...
which.input <- list(
  ...
  which.gene.set = c("immune.TF") # character
)
#c("P2RY2", "P2RX7", "P2RY1", "P2RY6")
...
for (i in 1:nrow(subset.input)){
  subset.list[[subset.input$subset[i]]] <- data.frame(
    ...
    subgroups =  stringr::str_trim(unlist(strsplit(subset.input$subgroups[i], ","))) 
  )
...
```{r seurat data subsetting , warning = FALSE}
...
for (i in 1:length(available_markers)){
  ...
  if (print.inline){
    plt.sgExp.all[[available_markers[i]]]
  }
}
```
