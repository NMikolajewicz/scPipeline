...
which.input <- list(
  ...
  query_input_format = 2,
  do.module = F, #options: T, F, NA; if NA, takes default, otherwise overides. 
  
  # specify gene set (only if query_input_format == 1 | 2 | 5 | 6)
  which.gene.set = "neftel.genes" # character (e.g., sex.specific)
)
# which.gene.set = c("Gpnmb", "Cd68",  "Bnc2", "Ptgs2", "Unc5b", "Twist2", "Efs", "Plxnb3", "Gjb3", "Lancl3", "Gas1", "Pdpn", "Stc2")
...
which.results <- data.frame(
  umap.top.n = 20,       # plot top n markers (by DEG analysis). NA if all. 
  cor.top.n = 70,
  gsea.top.n = 20,
...
correlation.settings <- list(
  which.data = "scale",                     # which data used as input for correlations; options: 'data' or 'scale'
  check.existing = T,                        # check if correlation matrix was precomputed (M27)
  recompute.existing.if.query.missing = F    # if pre-computed corr matrix exists, but query is missing, recompute?
)
...
subset.input <- read.csv("M00_subgroups.csv", header = TRUE, stringsAsFactors = F)
...
subset.df <- subset.list[[subgroup]] # NA specified as "no.subset"
```
...
for (i in 1:length(available_markers)){
  ...
  plt.sgExp.clust <- expression.Plot(so = so.query, e.mat = e.mat, f.mat = f.mat.clust, 
                                         which.gene = available_markers[i], which.group = "seurat_clusters", which.data = "data") + 
    ylab(paste0(available_markers[i], " Expression")) + xlab("Cluster ID") + 
    labs(title = "Violin Plots", subtitle = "Clusters")  +  theme_miko(legend = F)
  
  is.success <- F
  try({
    plt.sgExp.barcode <- expression.Plot(so = so.query, e.mat = e.mat, f.mat = f.mat.bc, 
                                             which.gene = available_markers[i], which.group = "Barcode", which.data = "data") +  
      ylab(paste0(available_markers[i], " Expression"))  + xlab("Sample ID") + 
      labs(title = "", subtitle = "Samples")   +  theme_miko(legend = F) + 
...
if (ncol(query.cor.mat.all) == 0) stop("Correlation matrix is missing query features. If precomputed cor mat (M27) was used, try recomputing in current module (M09)")
...
```{r combine expression plots, fig.height=10, fig.width=30, include = FALSE}
...
for (i in 1:length(available_markers)){
  ...
  lq <- cowplot::plot_grid(plotlist = list(top.lq, low.lq), ncol = 1, rel_heights = c(2,2))
