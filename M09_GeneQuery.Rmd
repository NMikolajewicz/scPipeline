...
which.input <- list(
  ...
  which.gene.set = c("Gpnmb", "Cd68",  "Bnc2", "Ptgs2", "Unc5b", "Twist2", "Efs", "Plxnb3", "Gjb3", "Lancl3", "Gas1", "Pdpn", "Stc2") # character
)
...
#input.file <- "Module1_M16_M27_UHN_240620.Rdata"
...
if (!exists("input.file")) stop("input.file not specified.")

# import and preprocess data
warning("Importing data...")
if (!grepl(".Rdata|.RData", input.file)) input.file <- paste0(input.file, ".Rdata")
load(getLoadPath(input.file, paste0(data.path, dir.preprocessed)))
...
if (subgroup != "no.subset") so.query <- subsetSeurat(so.query, subset.df)
...
# check species
detected.species <- unique(so.query@meta.data[["Organism"]])
if (detected.species != default.species) {
  if (length(detected.species) == 1) {
    which.species <- detected.species
    warning("Incorrect input species was provided, and was updated to reflect what was available in seurat object.\n")
  } else if (length(detected.species) > 1)  {
    which.species <- default.species
