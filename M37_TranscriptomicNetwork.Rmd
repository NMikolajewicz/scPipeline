...
parameter.list <- list(
  ...
  subsample_factor = 1,
  # subset.data = data.frame(field = "Barcode", subgroups = "GL261"),
  subset.data = NA,
  species = "Mm",
  min.pct = 0.5, # large net: 0.1; small net: 0.5 -  min gene expression (applicable only if variable.only = F)
  variable.only = T, # variable genes only?
...
# 
# paste0(format(Sys.time(), "%d_%b_"), gsub(" ", "_", gsub(":", "_", format(Sys.time(), "%X"))), format(Sys.time(), "_%Y"))
# 
...
df.wnn.umap.meso.sum  <- df.wnn.umap.meso %>%
  ...
  dplyr::summarize(x.mean = median(x), y.mean = median(y))
...
plt.hm.tf <- maHM(cell.object = so.query, df.wnn.umap = df.wnn.umap.tf, mod.res = mod.res.tf, k.row = 3, k.col = 3)
plt.hm.meso <- maHM(cell.object = so.query, df.wnn.umap = df.wnn.umap.meso, mod.res = mod.res.meso, k.row = 3, k.col = 5)
...
General Network | Graph & Annotation
===================================== 
