...
scaleFreeNet <- function(object, graph.name = "RNA_snn", n.dim, n.workers = 4){
  ...
  message("Identifying optimal power...")
  ...
  message("\tOptimal Power: ",  st.res$powerEstimate)
...
# general network
message(paste0("Generating general network using ", ncol(expr.mat), " genes..."))
...
# TF-network only
message(paste0("Generating transcription factor network using ", ncol(expr.mat.tf), " genes..."))
suppressMessages(suppressWarnings({
...
prepConnectivityObjects <- function(gene.object){
  ...
  graph.threshold <- quantile(snn.graph[snn.graph > 0], 0.8)
  df.graph.edges <- NULL
...
for (i in 1:length(gene.list)){
 ...
 plt.mod.umap <-  connectivity.plot + 
   ...
   scale_color_manual(values = c("TRUE" = "grey", "FALSE" = "grey80")) + 
   scale_size(range = c(0.5,5)) + 
   ggrepel::geom_text_repel(data = df.umap.gene.top, aes(x = x,  y =y, label = gene), inherit.aes = F, segment.alpha = 0.15, size = 3)
