...
# load packages
packages2load <- c("scMiko", "Seurat", "plyr",  "dplyr", "tidyr", "reshape2", "gridExtra", "WGCNA", "topGO", "ape", "flashClust",
...
# input.file <- "R44_M27_NM2_p4789_allGBM_immuneSubpop_140820.Rdata"
...
prep.list <- prepSeurat2(so, e2s = gNames.list, 
                             species = which.species, resolution= cluster.resolution, subset.data = subgroup, 
                             subsample = subsample_factor, M00_subgroup.path = "M00_subgroups.csv", rmv.pattern = "so")
...
savePDF(file.name = paste0(output.path, "PDF/", "M24_gene_network_clusters_umap.pdf"), 
        plot.handle =  plt.umap.mod.clusters, save.flag = save.pdf, fig.width = 7, fig.height = 5)
...
if (length(plt.umap.mod.list) > 0){
  invisible({
    for (i in 1:length(plt.umap.mod.list)){
      ...
      savePDF(file.name = paste0(output.path, "PDF/", plot.name), 
              plot.handle =  plt.umap.mod.list[[i]], save.flag = save.pdf, fig.width = 7, fig.height = 5)
    }
  })
}
