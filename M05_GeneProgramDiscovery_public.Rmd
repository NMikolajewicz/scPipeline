...
packages2load <- c("Seurat", "scMiko", "DT", "scales", "STRINGdb","dplyr", "NNLM", "BiocGenerics", "IRanges", "AnnotationDbi","tidyr", "RColorBrewer", "ggplot2", "heatmaply", "htmltools","flexdashboard", "future", "BiocParallel","parallel", "doParallel", "foreach", "iterators")
...
parameter.list <- list(
  ...
  subset.data = NA,
  batch.feature = NA, 
  feature.selection.method = c("expr"),        # options: expr, hvg, deviance 
  max.features = 2000, 
  min.pct = 0.5, 
...
For details on the **Gene Program Discovery** scPipeline module, see [documentation](https://nmikolajewicz.github.io/scMiko/articles/scPipeline_module_overview.html).\

---

**Description**: Sample overview and summary of STRING protein-protein interactions (PPI) for gene programs identified by SSN analysis.

---

**UMAP**. UMAP representation of scRNA-seq sample.\

```{asis, echo = (ppi.success == T)}
**PPI enrichment plot**. Enrichment of known protein-protein interactions (PPI) among gene programs identified by SSN analysis. [STRING database](https://string-db.org/) was used for enrichment analysis.
```
...
**Description**: Functional annotation of gene programs 
...
**Description** Summary of gene program similarities and composition. 

---

**Gene set similarity matrix**. Heatmap(s) visualizing Jaccard similarity between gene programs discovered by different methods. If only one gene program discovery method was used, this result is not shown.\

**SSN network layout**. UMAP coordinates and gene program membership and connectivity of each gene.\
