...
save.filename <- "celsius_renca_rpca_filtered_030321"
...
for (i in 1:length(input.data)){
  ...
  set.name <- names(input.data)[i]
  
  if (is.na(input.data[[set.name]]$integrate.by)) next
  
  
  integrate.by <- input.data[[set.name]]$integrate.by
  message(paste0("Splitting '", set.name, "' by '", integrate.by, "'..."))
  input.data.current <- input.data[[set.name]]
...
# warning("Identifying top candidate genes for integration...\n")
  ...
  # var.list <- purrr::map(so.list, function(x) data.frame(genes = rownames(x@assays[[DefaultAssay(x)]]@meta.features),
  #                                          var = x@assays[[DefaultAssay(x)]]@meta.features[["sct.residual_variance"]]))
...
if (length(so.list) > 10){
  plt.pie <- plt.pie + theme(legend.position = "none")
}
...
message("Selecting integration features...")
  ...
  so.list2 <- PrepSCTIntegration(object.list = so.list[so.n > k.filter ], anchor.features = so.features, verbose = FALSE)
