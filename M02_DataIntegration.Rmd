...
n.anchors <- 2000  # number of features used for data integration # if datasets are divergent, may need fewer genes to align with (e.g., 1000)

integration_method <- "rPCA" # "CCA" for small datasets, "rPCA" for large datasets
...
n.workers <- list(
  ...
  do.memory.limit <- T # specify whether to limit max memory (set T if local)
  max.memory <- 150 # numeric, in terms of Gb (set to 100 for cluster)
...
#     groups = NA),
#   nuc_09_T1 = list(
#     file = "Module1_09-CID003556-filtered_190220.Rdata",
#     cluster.resolution = 0.3,
#     # integrate.by = NA,
#     species = "Mm",
#     subsample = subset.all,
...
#     groups = NA),
#   nuc_11_T2 = list(
#     file = "Module1_11-CID003558-filtered_190220.Rdata",
#     cluster.resolution = 0.3,
#     species = "Mm",
#     subsample = subset.all,
#     grouping.field = NA,
#     groups = NA),
#   cell_10_T1 = list(
#     file = "Module1_10-CID003557-filtered_190220.Rdata",
#     cluster.resolution = 0.3,
#     species = "Mm",
#     subsample = subset.all,
#     grouping.field = NA,
#     groups = NA),
#   cell_12_T2 = list(
#     file = "Module1_12-CID003559-filtered_190220.Rdata",
#     cluster.resolution = 0.3,
#     species = "Mm",
#     subsample = subset.all,
#     grouping.field = NA,
#     groups = NA),
#   sci_T1 = list(
#     file = "Module1_Celsuis_sci_Renca_tumorOnly_T1_230220.Rdata",
#     cluster.resolution = 0.3,
#     species = "Mm",
#     subsample = subset.all,
#     grouping.field = NA,
#     groups = NA),
#   sci_T2 = list(
#     file = "Module1_Celsuis_sci_Renca_tumorOnly_T2_230220.Rdata",
#     cluster.resolution = 0.3,
#     species = "Mm",
#     subsample = subset.all,
#     grouping.field = NA,
#     groups = NA)
#   # p13.renca = list(
#   #   file = "R398_M01_NM2_pilot13_celsius_renca_230221.Rdata",
#   #   cluster.resolution = 0.3,
#   #   species = "Mm",
#   #   subsample = subset.all,
#   #   grouping.field = NA,
...
```{r update variable features}

# n.anchors <- 2000
# n.anchors2 <- round(1.5 * n.anchors)

for (i in 1:length(so.list)){
  
  if (!("SCT" %in% names(so.list[[i]]@assays))) stop("All objects must have 'SCT' assay. Other normalization pipelines are currently not supported.")
  if (class(so.list[[i]]@assays[["SCT"]]) == "SCTAssay"){
    df.var <-  so.list[[i]]@assays[["SCT"]]@SCTModel.list[["model1"]]@feature.attributes
    rownames(df.var) <- make.unique(df.var$SYMBOL)
    so.list[[i]]@assays[["SCT"]]@SCTModel.list[["model1"]]@feature.attributes <- df.var
    df.var <- df.var %>% dplyr::arrange(-residual_variance)
     so.list[[i]]@assays[["SCT"]]@var.features <- df.var$SYMBOL[1:round(1.5 * n.anchors)]   
  } else {
...
message("Selecting integration features...")
  ...
  which.too.small <- names(so.n)[so.n <= k.filter]
  if (length(which.too.small) > 0){
    message(paste0(which.too.small, collapse = ", "), " were removed due to insufficient number of cells (n < ", k.filter, ")")
    n.datasets <- sum(so.n > k.filter)
  }
  
  
  # debugonce(PrepSCTIntegration)
  so.list2 <- PrepSCTIntegration(object.list = so.list[so.n > k.filter ], anchor.features = so.features, verbose = T)
...
so <- FindClusters(object = so, resolution = target.cluster.resolution, verbose = 0, algorithm = 1, modularity.fxn = 1, group.singletons = T)
...
try({
  savePDF(file.name = paste0(output.path, "PDF/", "M02_UMAP_input_vs_integrated.pdf"), 
          plot.handle = umap.oi, 
          fig.width = 12, fig.height = 7, save.flag = save.pdf)
}, silent = T)

```

### Integration by batch
```{r plt.integration_by_batch1, fig.height=15, fig.width=14}
...
try({
  savePDF(file.name = paste0(output.path, "PDF/", "M02_UMAP_batch.pdf"), 
          ...
          fig.width = 14, fig.height = 15, save.flag = save.pdf)
}, silent = T)
}
...
```{r plt.integration_by_batch2, fig.height=10, fig.width=14}

if ((n.datasets > 10) & (n.datasets <= 20)){
  print(plt.integration_by_batch)
...
```{r plt.integration_by_batch3, fig.height=6, fig.width=14}
...
try({
  savePDF(file.name = paste0(output.path, "PDF/", "M02_UMAP_batch.pdf"), 
          plot.handle = plt.integration_by_batch, 
          fig.width = 14, fig.height = 6, save.flag = save.pdf)
}, silent = T)
}
...
plt.umap_by_barcode <- plt.umap_by_barcode + 
  ...
  scale_color_manual(values = categoricalColPal(n = n.datasets)) + 
  theme(legend.position = "bottom")
print(plt.umap_by_barcode)
...
```{r, fig.height=7, fig.width=12, dpi = 72}
print(plt.bulk.comparison)
...
plt.comp.combo <- cowplot::plot_grid(plt.umap_by_cluster + theme(legend.position = "none"), plt.cluster_composition, rel_widths = c(1,1.5), align = "h")
