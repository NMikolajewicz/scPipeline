...
save.filename <- "M02_testSet_rPCA_120820"
...
n.workers <- list(
  ...
  max.memory <- 100 # numeric, in terms of Gb (set to 100 for cluster)
...
for (i in 1:length(so.list)){
  ...
  so.list[[set.name]] <- ens2sym.so(so = so.list[[set.name]], gNames.list = gList[[set.name]])
