---
title: "scRNAseq: Data Integration"
output: 
...
save.filename <- "Module2_p4_p7_KRBtest_200720"
...
n.workers <- list(
  ...
  max.memory <- 25 # numeric, in terms of Gb (set to 100 for cluster)
...
input.data <- list(
  p4.GL261_01 = list(
    ...
    grouping.field = "Barcode",
    groups = c("01_G(GL261)_FR_CST","02_C(CT2A)_FR_CST","03_H(GL261)_FR_CSTsi")),
  p7.CT2A_03_R1_GL261_04_R1 = list(
    file = "Module1_p7_GBM_080720.Rdata",
    cluster.resolution = 1,
    species = "Mm",
    subsample = subset.all,
    grouping.field = "Barcode",
    groups = c("03_GBM_CT2A","04_GBM_GL261"))
)


```

```{r alternative input datasets}
...
#     subsample = subset.all,
#     grouping.field = "Barcode",
#     groups = c("03_GBM_CT2A")),
#   p8.GBM_PBSctrl_06 = list(
#     file = "Module1_p8_GL261_tumor_090720.Rdata",
#     cluster.resolution = 1,
#     species = "Mm",
#     subsample = subset.all,
#     grouping.field = "Barcode",
#     groups = c("06_GBM_PBSctrl")),
#   p8.CT2A_01_R2 = list(
...
#     subsample = subset.all,
#     grouping.field = "Barcode",
#     groups = c("04_GL261_R3")),
#   p8.GL261.GFP_05 = list(
#     file = "Module1_p8_GL261_tumor_090720.Rdata",
#     cluster.resolution = 1,
#     species = "Mm",
#     subsample = subset.all,
#     grouping.field = "Barcode",
#     groups = c("05_GL261GFP")),
#   p8.GBM_PBSctrl_06 = list(
#     file = "Module1_p8_GL261_tumor_090720.Rdata",
#     cluster.resolution = 1,
#     species = "Mm",
...
```{r convert gene symbols to species-appropriate notation}
# species-specific gene symbols
# genes4species <- function(x, input.species, target.species) {
#   if (input.species == "Mm" & target.species == "Hs") {
#     y <- toupper(x)
#   } else if (input.species == "Hs" &  target.species == "Mm") {
#     y <- tolower(x)
...
input.files <- paste0(dir, input.files)
...
import.list <- foreach(i = 1:length(input.files), .packages = c("scMiko"))  %dopar% {
...
ens2sym.so.dev <- function(so, gNames.list, convert.RNA = TRUE){
...
for (i in 1:length(so.list)){
  ...
  if (!(sum(which.match) == length(gene.rep))) {
    so.list[[set.name]] <- subset(so.list[[set.name]], features = gene.rep[which.match])
  }
  
  so.list[[set.name]] <- ens2sym.so.dev(so = so.list[[set.name]], gNames.list = gList[[set.name]])
  
  # gList.consolidate <- c(gList.consolidate, gList[[set.name]])
...
# so.orig <- mergeSeuratList(so.list)
...
warning("Selecting integration features...")
  ...
  so <- IntegrateData( 
    ...
    features.to.integrate = which.features.integrate,
    verbose = T
  )
...
```{r # prep post integration values, fig.width = 12, fig.height = 4}
...
### Integration
```{r plt.integration, fig.width=10, fig.height=4, dpi = 72}


print(cowplot::plot_grid(plt.umap.original, plt.integration, ncol = 2))
```

### Integration by batch
```{r plt.integration_by_batch, fig.height=4, fig.width=12, dpi = 72}
print(plt.integration_by_batch)
```
...
```{r plt.PCA, fig.width=10, fig.height=4, dpi = 72}
...
cowplot::plot_grid(plt.umap_by_cluster, plt.cluster_composition)
```
...
if (save.flag == TRUE){
  if (!exists("dir")){dir <- ""} 
  save(so, gNames.list, df.log_Module_2, file = paste0(dir, save.filename))
}
