...
save.filename <- "M02_neuroDif_sct_p1011_011020"
...
n.workers <- list(
  import = 6,
  integration = 4 #(limit to 1 for cluster)
)
...
# drop scaled data to allow computational scaling
# for (i in 1:length(so.list)){
#   try({so.list[[i]]@assays[["RNA"]]@scale.data <- matrix(NA)}, silent = T)
#   try({so.list[[i]]@assays[["SCT"]]@scale.data <- matrix(NA)}, silent = T)
#   try({so.list[[i]]@assays[["integrated"]]@scale.data <- matrix(NA)}, silent = T) 
# }
# 
# invisible({gc()})
