...
# load packages
packages2load <- c("scMiko", "Seurat",  "biomaRt", "AnnotationDbi", "dplyr", "CONICSmat",
...
# Query input
input.file <- "R54_M27_NM2_p4789_allGBM_tumorSubpop_iter1_190820.Rdata"
...
# subset.df <- data.frame(field = "Barcode", subgroups = c("ctrl.0231", "TRA.0231", "TR.0231"))
subset.df <- data.frame(field = "Barcode", subgroups = c("ctrl.0238", "TR.0238"))
...
prep.list <- prepSeurat2(so, e2s = gNames.list, 
                             species = which.species, resolution= cluster.resolution, subset.data = NULL, 
...
try({df.log <- addLogEntry("Subsetting", (subset.df), df.log, "subset.df")}, silent = T)
...
```{r helper function (adopted from CONICSmat)}

geneChrPosition <- function (gene_names, ensembl_version = "dec2016.archive.ensembl.org", 
  species = "human") 
{
  if (species == "human") {
...
filterGenes <- function (mat1, genelist, minExp = 10, minCells = 5) 
{
  mat1 = mat1[intersect(genelist, rownames(mat1)), ]
  mat1 = mat1[which(rowSums(mat1) > minExp), ]
  percentgenes = apply(mat1, 1, function(x) {
    sum(x > 0)
  })
...
# get barcodes
so.annot <- so.query@meta.data[["Barcode"]]

# e.mat <- as.matrix(Seurat::GetAssayData(so.query, assay = DefaultAssay(so.query), slot = "data"))
e.mat <- as.matrix(so.query@assays[["RNA"]]@scale.data)

n.cells <- 100
e.mat <- e.mat[ , sample(c(1:ncol(e.mat)), n.cells)]

# get regions
regions=read.table("M07_chromosome_arm_positions_grch38.txt",sep="\t",row.names = 1,header = T)

# get gene positions
gene_pos <- geneChrPosition(rownames(e.mat), species = if (which.species == "Hs") "human" else "mouse")

# filter matrix
# e.mat <- filterGenes(e.mat,gene_pos[,2],minCells=5)

# get cluster averages
df.meta <- so.query@meta.data
u.clust <- getOrderedGroups(so.query, which.group = "seurat_clusters", is.number = T)
c.mat <- matrix(nrow = nrow(e.mat), ncol = length(u.clust))
# for (i in 1:length(u.clust)){
#   which.cells <- rownames(df.meta)[df.meta$seurat_clusters %in% u.clust[i]]
#   e.mat.sub <- e.mat[ , colnames(e.mat) %in% which.cells]
#   c.mat[,i] <- rowMeans(e.mat.sub)
# }
...
rownames(c.mat) <- rownames(e.mat)
# colnames(c.mat) <- paste0("c", u.clust)

# sort genes and fit chr-level loess
chr.seg <- rownames(regions)

  co.mat <- NULL
  for (i in 1:nrow(regions)){
  current.seg <- chr.seg[i]
  chr.genes <- gene_pos[gene_pos$chromosome_name %in% regions$Chrom[i], 2]
  
  if (length(chr.genes) == 0) next
  c.mat.sub <- c.mat[rownames(c.mat) %in% chr.genes, ]
  
  df.query <- data.frame(genes = rownames(c.mat.sub), ind = c(1:nrow(c.mat.sub)))
  df.ref <- data.frame(genes = gene_pos[,2], start.pos = gene_pos$start_position)
  df.match <- merge(df.query,df.ref, by = "genes")
  df.match <- df.match %>% dplyr::arrange(start.pos)
  
  c.mat.sub <- c.mat.sub[df.match$ind, ]
  
  co.mat <- rbind(co.mat, c.mat.sub)

  }
  
  df.cnv.all <- NULL
  df.cnv <- NULL
  for (j in 1:ncol(co.mat)){
    
    print(paste0("Progress: ",j, "..."))
    # df.data <- data.frame(x = c(1:nrow(co.mat)), y = (co.mat[, j]), genes = rownames(co.mat))
    # 
    # loess.span <- 50
    # error.loess.mean <- loess(y ~ x, df.data, span = loess.span, degree = 2)
    # pred <- predict(error.loess.mean, data.frame(x = df.data$x), se = F)
    
    pred <- zoo::rollmean(co.mat[, j], k= 100)
    
    df.cnv <- bind_cols(df.cnv, data.frame(pred))
  }
  
  colnames(df.cnv) <-  colnames(co.mat)
  df.cnv.all <- bind_rows(df.cnv.all, df.cnv)

  # which.ref <- c("c4")
  # which.match <-  which(colnames(df.cnv.all) %in% which.ref)
  
  # mean.ref <- df.cnv.all[ ,which.match]
  # mean.ref <- apply(df.cnv.all[ ,which.match], 1, function(x) mean(x))
  # ci.thresh.ref <- apply(df.cnv.all[ ,which.match], 1, function(x) sd(x))
  # ci.thresh.ref <- 0.15
  
  # df.cnv.thresh <- df.cnv.all - mean.ref
  # df.cnv.thresh[abs(df.cnv.thresh) < ci.thresh.ref] <- 0
  
# color = viridis::inferno(20),
  
 df.cnv.all.t <- t(df.cnv.all)
  # df.cnv.all.t <- t(df.cnv.thresh)
 df.cnv.all.t <- t(apply(df.cnv.all.t, 1, function(x) (x-mean(x))/sd(x)))
 df.cnv.all.t <- (apply(df.cnv.all.t, 2, function(x) (x-mean(x))/sd(x)))
 
 threshold.score <- 3
 df.cnv.all.t <- (df.cnv.all.t^2) *sign(df.cnv.all.t)
 df.cnv.all.t[df.cnv.all.t > threshold.score] <- threshold.score
 df.cnv.all.t[df.cnv.all.t < -1*threshold.score] <- -1*threshold.score
 # df.cnv.all.t[abs(df.cnv.all.t) < threshold.score] <- 0
 
  pheatmap::pheatmap(df.cnv.all.t, cluster_rows =T, cluster_cols = F,  scale = "none", show_rownames = F)
...
# e.mat <- CONICSmat::normMat(as.matrix(Seurat::GetAssayData(so.query, assay = DefaultAssay(so.query), slot = "counts")))
  e.mat <- log1p(c.mat)

# Neftel 2019 preprocessing (should be barcodes)
...
# get barcodes
# so.annot <- so.query@meta.data[["Barcode"]]
  so.annot <- colnames(c.mat)

# get regions
regions=read.table("M07_chromosome_arm_positions_grch38.txt",sep="\t",row.names = 1,header = T)

# get gene positions
gene_pos <- getGenePositions(rownames(e.mat), species = if (which.species == "Hs") "human" else "mouse")

# filter matrix
# e.mat <- filterMatrix(e.mat,gene_pos[,2],minCells=5)
# e.mat <- filterMatrix(e.mat,gene_pos[,2],minCells=0)

# norm factors
normFactor <- calcNormFactors(e.mat)

#  specify output names
output.name <- "clusterAgg_v3"

# initial run
conic.plot <- plotAll(e.mat,normFactor,regions,gene_pos,output.name)

# visualize clusters
conic.hm  <- plotHistogram(conic.plot,e.mat,clusters=3,zscoreThreshold=2,so.annot)

# identify candidates
lrbic <- read.table(paste0(output.name, "_BIC_LR.txt"), sep = "\t", header = T, row.names = 1, check.names = F)

# default settings = bic dif 200, pval 0.01
# which(lrbic[,"BIC difference"]>200 & 
candRegions <- rownames(lrbic)[lrbic[,"LRT adj. p-val"]<0.5]

# refined run
conic.hm2  <- plotHistogram(conic.plot[,candRegions],e.mat,clusters=3,zscoreThreshold=3,so.annot)

normal <- which(conic.hm2 == 2)
tumor <- which(conic.hm2 == 1)

redu=plotAll(e.mat,normFactor,regions[candRegions,],gene_pos,output.name,normal=normal,tumor=tumor)


plotAllChromosomes (mat = e.mat,normal = normal,tumor = tumor,windowsize = 101,gene_pos = gene_pos,fname = paste0(output.name, "v2)"),patients = so.annot,breakpoints = regions)

# hist(lrbic$`BIC difference`)
plotChromosomeHeatmap(e.mat,normal = normal, plotcells = tumor, gene_pos = gene_pos, windowsize = 121, chr=T, expThresh=1, thresh = 1)
}

# plotChromosomeHeatmap(e.mat,normal = set1, plotcells =set2, gene_pos = gene_pos, windowsize = 121, chr=T, expThresh=0.2, thresh = 1)
...
# e.mat <- as.data.frame(so.query@assays[[DefaultAssay(so.query)]]@counts)
  e.mat <- c.mat
...
so.annot <-  data.frame(V2 = colnames(e.mat))
# so.annot <-  data.frame(V2 = so.query@meta.data[["Barcode"]])
 rownames(so.annot) <- colnames(e.mat)

# create InferCNV Object
infercnv_obj <- CreateInfercnvObject(
  raw_counts_matrix = as.matrix(log1p(e.mat)),
