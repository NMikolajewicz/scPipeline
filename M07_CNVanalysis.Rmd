...
so.query <- setResolution(so = so.query, cluster.resolution)
...
```{r prep gene list, warning = FALSE}
...
# get gene positions
gene_pos <- getGenePositions(rownames(e.mat))

# filter matrix
e.mat <- filterMatrix(e.mat,gene_pos[,"hgnc_symbol"],minCells=5)
...
# visualize clusters
conic.hm  <- plotHistogram(conic.plot,e.mat,clusters=2,zscoreThreshold=2,so.annot)
...
# refined run
conic.hm2  <- plotHistogram(conic.plot[,candRegions],e.mat,clusters=2,zscoreThreshold=4,so.annot)
...
plotAllChromosomes (mat = e.mat,normal = normal,tumor = tumor,windowsize = 101,gene_pos = gene_pos,fname = output.name,patients = so.annot,breakpoints = regions)
