---
title: "Module24_WGCNA_NetworkModules"
output: 
...
subset.list <- list(
  ...
  p4.gbm.scaled.c10 = data.frame(field = "seurat_clusters",
                                 subgroups = c(10)),
...
# query.file <- "Module1_m27_p7_mesoderm_070420.Rdata"
query.file <- "Module1_M16_M27_UHN_070620.Rdata"
...
# List of additional genes to include
...
```{r GENERAL - import seurat object}

# import and preprocess data
load(getLoadPath(query.file, dir.preprocessed))

if (exists("so.query")){
  so.query <- prepSeurat(so.query)
} else if (exists("so")){
...
```{r GENERAL - get clusters and assay}
...
```{r GENERAL - seurat data subsetting }
...
```{r GENERAL - prep seurat  gene list}
gNames.list <- prepGeneList(so.query, objects())
```



```{r GENERAL - convert to symbol}
...
```{r GENERAL - get clusters and set assay}

if (DefaultAssay(so.query) == "integrated") DefaultAssay(so.query) <- "SCT"
...
```{r GENERAL - downsample Seurat, warning = FALSE}

if (exists("downsample.factor")) so.query <- downsampleSeurat(so.query, downsample.factor)
...
```{r}
e.mat <- getExpressionMatrix(so.query, which.data = "scale", which.assay = "SCT")

ef.mat <- e.mat[tolower(rownames(e.mat)) %in% tolower(c("FZD2", "FZD7")), ]

df.fz <- data.frame(t(ef.mat))
df.fz$cell <- rownames(df.fz)
df.fz$cluster <- so.query@meta.data[["seurat_clusters"]]


df.fz$exp.detected <- "none"
df.fz$exp.detected[(df.fz$FZD7 > 0) & (df.fz$FZD2 > 0)] <- "FZD2 + FZD7"
df.fz$exp.detected[(df.fz$FZD7 > 0) & (df.fz$FZD2 == 0)] <- "FZD7"
df.fz$exp.detected[(df.fz$FZD7 == 0) & (df.fz$FZD2 > 0)] <- "FZD7"


df.fz.table <- data.frame(table(df.fz$exp.detected))

df.fz.subgroup <- df.fz %>%
  group_by(cluster, exp.detected) %>%
  summarize(n = length(exp.detected))


plt.fzd <- df.fz.subgroup %>%
  ggplot(aes(x = cluster, y = n, fill = exp.detected)) + 
  geom_bar(stat = "identity", position="fill") + 
  labs(title = "FZD2 and FZD7 Expression",
       subtitle = "Expressing Fraction") + 
  scale_fill_viridis_d("FZD Expression", begin = 0.2, end = 1, option = "E") +
  theme_miko(legend = T) + 
...
# entire matrix
exp.mat.complete <- getExpressionMatrix(so.query, which.data = "scale")

if ("similarity.scale" %in% names(so.query@misc)){
  s.mat <- so.query@misc[["similarity.scale"]][["similarity.matrix"]]
  s.mat <- s.mat[!duplicated(rownames(s.mat)), !duplicated(colnames(s.mat))]
  so.query@misc[["similarity.scale"]][["similarity.matrix"]] <- s.mat
  ...
  if (use.var){
    exp.mat <- getExpressionMatrix(so.query, only.variable = use.var, which.data = "scale", use.additional.genes = use.additional.genes)
  } else {
...
if ("similarity.scale" %in% names(so.query@misc)){
  which.cor <- so.query@misc[["similarity.scale"]][["metric"]]
  s.mat <- so.query@misc[["similarity.scale"]][["similarity.matrix"]]
...
plt.rank.connectivity.1 <- df.con.1 %>%
  ...
  geom_text_repel(aes(rank, log10(wi+1), label =ifelse(label ,as.character(genes),'')))
...
for (i in 1:length(u.mod)){
  ...
  # statistics
  cur.df.noG <- cur.df %>% select(-c("genes"))
...
getICAGenes <- function(feature.loading, fdr.cutoff = 0.0001, local.fdr = T, assert.positive.skew = T, only.pos = T){
  ...
  # ica.kme <- so.query@reductions[["ica"]]@feature.loadings
  ...
  # n.sig.genes <- apply(ica.p, 2, function(x) sum(x < fdr.cutoff))
...
if (print.inline){
  pheatmap::pheatmap(ica.cluster.activity, main = "ICA Pathway Similarities", scale  = "column")
}
...
if (print.inline){
  ...
  ggVennDiagram::ggVennDiagram(ica.module.genes[1:4]) 
  j.mat <- jaccardSimilarityMatrix(gene.sets = ica.module.genes, assert.unique = T)
...
if (print.inline){
  ...
  ggVennDiagram::ggVennDiagram(nmf.module.genes[1:4]) 
...
# colnames(nmf.fuz.df) <- paste0("NMF_", c(1:ncol(nmf.fuz.df)))
...
df.module.avg <- df.module.avg %>% select(-c("seurat_clusters"))
...
```{r reembed UMAP with modular definitions}

# get GGplot handle for cluster umap
plt.umap_by_cluster <- cluster.UMAP(so.query, group.by = "seurat_clusters", pt.size = T)

# create copy of original seurat
so.orig <- so.query

wgcna.reduction <- F # assume T as default always

if (wgcna.reduction){
  
  # store WGCNA reductions
  ME.f.dm <- ME.wgcna
  colnames(ME.f.dm) <- paste0("ME_", 1:ncol(ME.f.dm))
  ME.f.dm <- as.matrix(ME.f.dm)
  so.query[["WGCNA"]] <- CreateDimReducObject(embeddings = ME.f.dm, key = "ME_", assay = DefaultAssay(so.query))
  
  # run UMAP on WGCNA modular data
  so.query <- RunUMAP(so.query, dims = 1:ncol(ME.f.dm), min.dist = 0.75, reduction = "WGCNA", verbose = F)
  
  # UMAP using original cluster IDs
  plt.wgcna.umap <- cluster.UMAP(so.query)
  
  # recluster data using new embedding
  so.v2 <- FindNeighbors(so.query, reduction = "WGCNA", dims = 1:ncol(ME.f.dm), nn.eps = 0.5, verbose = F)
  so.v2 <- FindClusters(so.v2, resolution = cluster.resolution, n.start = 10, verbose = F)
  
  
  # map new clusters to old embedding
  df.meta.orig <- so.orig@meta.data
  df.meta.orig$new.clusters <-  so.v2@meta.data$seurat_clusters
  so.orig@meta.data <- df.meta.orig
  
  plt.wgcna.umap_w <- cluster.UMAP(so.v2)
  plt.umap_by_cluster_w <- cluster.UMAP(so.orig, group.by = "new.clusters")
  
  if (print.inline){
    cowplot::plot_grid(plt.umap_by_cluster_w + labs(title = "UMAP (PCA)",
                                                    subtitle = "WGCNA-based clustering"), 
                       plt.wgcna.umap_w + labs(title = "UMAP (WGCNA)", 
                                               subtitle = "WGCNA-based clustering"))
    
    cowplot::plot_grid(plt.umap_by_cluster + labs(title = "UMAP (PCA)", 
                                                  subtitle = "PCA-based clustering"), 
                       plt.wgcna.umap + labs(title = "UMAP (WGCNA)", 
                                             subtitle = "PCA-based clustering"))
  }
...
for (i in c(1:(length(markers_of_interest)))) {
  ...
  # 1) UMAP clusters
  all_plts[[1]]  <-   cluster.UMAP(so.orig, group.by = "seurat_clusters")
  
  # 2) UMAP expression
  cur.marker <- markers_of_interest[i]
  if (!is.na(cur.marker)) all_plts[[length(all_plts)+1]] <- scExpression.UMAP(so.orig, cur.marker) + 
...
plt.umap_by_cluster
# cowplot::plot_grid(plt.umap_by_cluster + ggtitle("UMAP (PCA)\nPCA-based clustering (original)"), 
#                    plt.wgcna.umap + ggtitle("UMAP (WGCNA)\nPCA-based clustering (original)"))

```



```{r plt.umap_by_w, fig.width=14, fig.height=5}

### WGCNA-based Clustering
# cowplot::plot_grid(plt.umap_by_cluster_w + ggtitle("UMAP (PCA)\nWGCNA-based clustering"), 
#                    plt.wgcna.umap_w + ggtitle("UMAP (WGCNA)\nWGCNA-based clustering"))
...
try({
  ...
  if (dim(df.module.avg)[1] > 1){
    # df.module.avg.mod <- df.module.avg
    # df.module.avg.mod <- as.data.frame(apply(as.matrix(df.module.avg.mod), 1, function(x) x/max(x)))
...
```{r}
   ggVennDiagram::ggVennDiagram(module.wgcna.list[1:4]) 
```
...
# df.log <- addLogEntry("Soft power (clusters)", best.power.HC, df.log, "best.power.HC")

# assay 
df.log <- addLogEntry("Seurat Assay", DefaultAssay(so.query), df.log, "DefaultAssay(so.query)")

# N modules detected 
df.log <- addLogEntry("Detected modules", length(master.module.list), df.log, "length(master.module.list)")
