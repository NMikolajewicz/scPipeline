...
which.references <- c("cancerSEA", "immune.signatures", "immuneSubtype", "GBM",  "MCA_50", "Zhang_2019", "Panglao.Mm",   "VastDB", "cycling.seurat", "immune.Monaco",  "invitro.GBM", "Zeisel2018_brainDev", "brain.GBM.R103", "GBM.invitro.fig1")

# , "invitro.GBM", "Zeisel2018_brainDev" "brain.GBM.R103",

# which.references <- "TRRUSTv2_Hs_reg"
# which.references <- "brain.GBM.R103", "neural.dif.HH"
...
for (i in 1:length(u.clusters)){
  ...
  if (print.inline) print( plt.cluster.umap[[paste0("c", cluster.name)]] )
}
...
for (i in 1:length(u.cl)){
  ...
  if (nrow(vst.merge.subset) >0){
    ...
    df.f1 <- vst.merge.subset %>%
      dplyr::top_n(20, composite)
    
    w1 <- df.f1 %>%
      ggplot(aes(label = cell.type, size = abs(composite))) +
      geom_text_wordcloud(color = df.f1$colorUp) +
      theme_minimal() + 
      labs(title  = "Enriched") + 
    ...
    w2 <- df.f2 %>%
      ggplot(aes(label = cell.type, size = abs(composite))) +
      geom_text_wordcloud(color = df.f2$colorDown) +
      theme_minimal() + 
      labs(title  = "Depleted") + 
      theme(plot.title = element_text(hjust = 0.5))
...
Annotation scores were loess-centered and variances were stabilized using 95% quantile B-spline regression to eliminate size-biased. These series of plots are used for diagnostic purposes. 
...
Annotation scores. Use Composition scores for classifying cell types. 
Abbreviations: EXP, Gene set (average) expression; GSVA, Gene set variation analysis; PER, Expressing Fraction (average); LOGFC, log fold-change; PD, difference between percentage of expression cells;  COMPOSITE, Average of component scores.
...
Cluster-level cell-type annotations. Red words: enriched annotations; Blue words: depleted annotations; Word size ~ composite score; Word transparency ~ confidence. Cluster of interest is highlighted in red on UMAP. 
