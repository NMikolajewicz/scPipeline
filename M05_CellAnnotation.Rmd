...
# slim down so
#so.query@assays[["RNA"]] <- NULL
#so.query@graphs[["integrated_nn"]] <- NULL
invisible(gc())
...
#################################################################################
# PARALLEL IMPLEMENTATION OF FINDALLMARKERS
# findAllMarkers.Parallel <- function(object, n.workers, ...){
#   
# }
...
if (do.deg){
  ...
  if (n.workers$deg == 1){
    ...
    M <- length(unique(so.query@meta.data$seurat_clusters))
    N <- M-1
    
    # get optimal BPParam setup
    if (n.workers$deg > M) n.workers$deg <- M
    nw <- safeBPParam(n.workers$deg)
    
    # find marker wrapper function
    FindMarker.wrapper <- function(object, x){
      Seurat::FindMarkers(object, ident.1 = x, only.pos = F, min.pct = 0, test.use = "MAST",
                          logfc.threshold = 0.15, max.cells.per.ident = 200)
    }
    
    # run parallelized implementation of find markers
    deg.gene.par <- BiocParallel::bplapply(0:N, object = so.query, FUN = FindMarker.wrapper, BPPARAM = nw)
    
    # retrieve results
    deg.gene.a <- NULL
    for (i in 1:length(deg.gene.par)){
      deg.gene.par[[i]]$cluster <- i-1
      deg.gene.par[[i]]$gene <- rownames(deg.gene.par[[i]])
      deg.gene.a <- bind_rows(deg.gene.a.unpack, deg.gene.par[[i]])
    }
...
# df.dist.mod <- wrangleHelper(mod.score.list)
...
```{r variance stabilizing score transform}
...
```{r score processing, warning = FALSE}
...
```{r deg evaluation, warning = FALSE}

# Find fraction of DEGs present in each geneset ##########################################

deg.gene.sub <- df.lfc.long[(df.pd.long$value > 0.3) & (df.lfc.long$value > 1) & (df.f.long$value > 0.3), ]
...
agg.mat <- aggGroupExpression(so.query, which.data = "scale")
...
for (i in 1:length(u.cl)){
  ...
  savePDF(file.name = paste0(output.path, "PDF/", plot.name), 
          plot.handle =  wordcloud::wordcloud(words = vst.merge.cloud[vst.merge.cloud$cluster %in% u.cl[i], ]$cell, 
                                              freq =  round((vst.merge.cloud[vst.merge.cloud$cluster %in% u.cl[i], ]$composite)*50), min.freq = 0,
                                              max.words=30, random.order=FALSE, rot.per=0, scale = c(1.5, .2),
                                              colors= vst.merge.cloud[vst.merge.cloud$cluster %in% u.cl[i], ]$color, ordered.colors = T), 
