...
cluster.resolution <- 0.15 # OPTIONAL; range 0 to inf (default = 1)

so.input.species <- "Hs" # REQUIRED; Hs, Mm
  ...
  if (length(sig) > 0){
    ...
    for (j in 1:length(sig)){
      ...
      if (sum(rownames(ef.mat) %in% sig[[j]]) > 0){
        ...
        per.mat.sub <- df.pd.long[df.pd.long$gene %in% cur.features, ]
        ...
        pd.exp <- as.vector(pct.mat.sub %>% group_by(cluster) %>% summarize(mean.exp= mean(value, na.rm = T)))
        pctdif.score[j,] <- as.vector(pd.exp$mean.exp)
        
        lfc.exp <- as.vector(lfc.mat.sub %>% group_by(cluster) %>% summarize(mean.exp= mean(value, na.rm = T)))
...
vstScoreThreshold.orig  <- function(x, y, label, set, percentile.threshold = 0.95, spline.df = 3, quantile.norm = T, loess.center = T){
  ...
  if (quantile.norm){
    ...
    rqfit <- rq(y.centered ~ bs(x, df = spline.df), data = df, tau = c(percentile.threshold))
    ...
    df$p.val <- 2*pnorm(abs(df$z.sig), lower.tail = F)
    df$p.adj <- p.adjust( df$p.val, method = "BH")
    df$sig <- (df$p.val < 0.05) & (df$z.sig > 0)
...
vstScoreThreshold  <- function(x, y, label, set, percentile.threshold = 0.95, spline.df = 3, quantile.norm = T, loess.center = T){
  ...
  if (quantile.norm){
    ...
    for (i in 1:length(uni.clust)){
      rqfit <- rq(y.centered ~ bs(x, df = spline.df), data = df[df$cluster %in% uni.clust[i], ], tau = c(percentile.threshold))
      df$qr95[df$cluster %in% uni.clust[i]] <- predict(rqfit, data.frame(x = (df$x[df$cluster %in% uni.clust[i]])))
      df$ratio[df$cluster %in% uni.clust[i]] <- df$y.centered[df$cluster %in% uni.clust[i]]/df$qr95[df$cluster %in% uni.clust[i]]
      df$residuals[df$cluster %in% uni.clust[i]] <- rqfit[["residuals"]]
      df$z.sig[df$cluster %in% uni.clust[i]]  <- (df$ratio[df$cluster %in% uni.clust[i]] - mean(df$ratio[df$cluster %in% uni.clust[i]] )) / sd(df$ratio[df$cluster %in% uni.clust[i]] )
      df$p.val[df$cluster %in% uni.clust[i]] <- 2*pnorm(abs(df$z.sig[df$cluster %in% uni.clust[i]]), lower.tail = F)
      # df$p.adj <- p.adjust( df$p.val, method = "BH")
      df$sigUp[df$cluster %in% uni.clust[i]] <- (df$p.val[df$cluster %in% uni.clust[i]] < 0.05) & (df$z.sig[df$cluster %in% uni.clust[i]] > 0)
      df$sigDown[df$cluster %in% uni.clust[i]] <- (df$p.val[df$cluster %in% uni.clust[i]] < 0.05) & (df$z.sig[df$cluster %in% uni.clust[i]] < 0)
    }
...
```{r score processing, warning = FALSE, fig.width=8, fig.height=6}
...
#   vst.merge.final$gen[i] <- paste(names(which(purrr::map_dbl(cell.types, 
#                                                              function(x) grepl(toupper(x), ann, fixed = F, perl = T)) == 1)), 
#                                   collapse = ", ")
# }
...
df.sig2plt <- unique((vst.merge.cloud %>% dplyr::filter(sig.lenient,composite > 1) %>% dplyr::arrange(-z.comp))[ ,c("cell.type", "ref.set")])
