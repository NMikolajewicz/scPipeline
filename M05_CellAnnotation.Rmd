...
# input.file <- "R35_M27_NM2_p4789_allGBM_uwPCA_130820.Rdata"
...
# "MCA_50", "Zhang_2019", "Panglao.Mm", "VastDB",  "cycling.seurat", 
which.references <- c("cancerSEA", "immune.signatures", "immuneSubtype", "GBM",  "MCA_50", "Zhang_2019", "Panglao.Mm", "VastDB",  "cycling.seurat", "neural.dif.HH")
...
df.s4plt <- as.matrix(df.s.wide %>% select(-c("gene")))
...
df.e4plt <- as.matrix(df.e.wide %>% select(-c("gene")))
...
#   
#   # cast as matrix
#   agg.mat <- as.matrix(agg.mat)
#   
#   # windorsize matrix
#   scale.target <- 4
#   agg.mat[(agg.mat > scale.target) | (agg.mat < (-1*scale.target))] <- scale.target
#   
#   plt.hm <- ggplotify::as.ggplot(pheatmap::pheatmap(agg.mat, show_rownames = F,
#                                                     main = "Expression Matrix\nx=clusters, y=genes, z=expression", scale = "none"))
#   
# } else {
#   plt.hm <- NULL
# }
