...
```{r prep matrix for expression heatmap}
...
write.csv(deg.gene.sub, file = paste0(output.path, "Tables/", "differential_genes_statistics.csv"), 
          row.names = F) 
...
try({
plt.hm <- ggplotify::as.ggplot( pheatmap::pheatmap(mat.exp.scale.v4,  breaks = seq(-3, 3, by = ((6)/100)), 
                   main = "Gene Expression Matrix\nrows=clusters, cols=genes, z=expression", show_rownames = F))
}, silent = T)
...
write.csv(mat.exp.scale.v4, file = paste0(output.path, "Tables/", "cluster_gene_expression.csv"), 
          row.names = T) 
