...
which.references <- c("cancerSEA", "immune.signatures", "immuneSubtype", "GBM",  "MCA_50", "Zhang_2019", "Panglao.Mm",   "VastDB", "cycling.seurat", "brain.GBM.R103", "neural.dif.HH", "immune.Monaco")
# which.references <- "brain.GBM.R103"

# specify cluster resolution
cluster.resolution <- 0.3 # OPTIONAL; range 0 to inf (default = 1)
...
prep.list <- prepSeurat2(so, e2s = gNames.list, 
                             species = so.input.species, resolution= cluster.resolution, subset.data = NULL, 
...
# 
# data_test <- function (file, most.variables = 0, lower = 0, upper = 0, normalize = TRUE,
#                        write = FALSE, verbose = TRUE, plot = FALSE)
# {
#   data<-as.matrix(file@assays[[DefaultAssay(file)]]@counts)
...
#     }
#   }
#   if (sum (all.genes %in% c(mm2Hs$`Mouse gene name`, mm2Hs$`Gene name`)) ==
#       0) {
#     cat("Please convert gene ID's (Ensembl or NCBI ID) to official HUGO gene symbols",
#         fill = TRUE)
#     return()
#   }
#   genes = all.genes
...
#     data.intermediate <- sweep(data, 2, q/median(q), "/")
#     data = log1p(data.intermediate)
#   }
#   data = data[rowSums(data) > 0, ]
#   data = data[rowSums(data) < quantile(rowSums(data), 1 -
#                                          upper) & rowSums(data) > quantile(rowSums(data), lower), ]
#   if (verbose == TRUE) {
#     cat(paste(dim(data)[1], "genes"), fill = TRUE)
#     cat(paste(dim(data)[2], "cells"), fill = TRUE)
#     cat(paste("Zero rate = ", round(sum(data == 0) * 1000/prod(dim(data)))/10,
#               "%", sep = ""), fill = TRUE)
#   }
#   if (write == TRUE) {
...
# small_so <- so.query
# data = data_test(small_so, write = FALSE)
# genes = rownames(data)
# # Proceed to clustering
# cluster <- as.character(small_so@meta.data[["seurat_clusters"]])
# cluster <- as.integer(cluster) + 1
# # clust = clustering(data = data,n = 10, method = "simlr")
# # cluster = clust$cluster
# # tsne = clust$`t-SNE`
# # Cell classification
# #my.markers = markers(c("immune", "tme", "melanoma", "bc"))
# # class = cell_classifier(data=data, genes=genes, plot.details = TRUE)
# # Cluster analysis
# # clust.ana = cluster_analysis(data = data, genes = genes, cluster = cluster)
# # Proceed to cell signaling
# # debug(cell_signaling)
# signal = cell_signaling(data = data, genes = genes, cluster = cluster,species = "mus musculus", logFC = log2(0.15), s.score = 0.5)
# # Visualization
# visualize(inter = signal)
# visualize(inter = signal, show.in = c(1))
# expression.plot(data = data, name = "LYPLA1", tsne = tsne)
# expression.plot.2(data = data, name.1 = "LYPLA1", name.2 = "TRAM1", tsne = tsne)
...
# try({es.mat <- so.query@assays[[DefaultAssay(so.query)]]@scale.data}, silent = T)
...
logfc.mat <- matrix(nrow = nrow(ef.mat), ncol = length(u.clust))
pct.mat <- matrix(nrow = nrow(ef.mat), ncol = length(u.clust))
e.mat <- matrix(nrow = nrow(ef.mat), ncol = length(u.clust))
# try({s.mat <- matrix(nrow = nrow(es.mat), ncol = length(u.clust))}, silent = T)
...
for (i in 1:length(u.clust)){
  ...
  if (n.cells.current == 0) stop(paste0("Cluster ",  u.clust[i], " has no cells associated with it."))
  
  # log data expression
  if (n.cells.current == 1){
  e1 <-  log(expm1(x = ef.mat[, which.cells])+ 1)
  } else { e1 <- log(rowMeans(expm1(x = ef.mat[, which.cells])) + 1)}
  e.mat[ ,i] <- e1
  
  # scaled data
  # try({
  #   if (n.cells.current == 1){
  #     s1 <- (es.mat[, which.cells]) 
  #   } else { s1 <- rowMeans(es.mat[, which.cells])}
  #   s.mat[ ,i] <- s1
  # }, silent = T)
  ...
  # expressing fraction
  if (n.cells.current == 1){
    f1 <- 1*(ef.mat[, which.cells] > 0)
  } else {f1 <- rowMeans(ef.mat[, which.cells] > 0) }
  f.mat[ ,i] <- f1
...
# try({df.s.wide <- data.frame(gene = rownames(es.mat), as.data.frame(s.mat)); colnames(df.s.wide) <- c("gene", u.clust)}, silent = T)
...
try({
  ...
  plt.heat.scaled <- pheatmap::pheatmap(df.s4plt,
                                        ...
                                        color = my.col,
                                        show_rownames = F,
                                        main = "Scaled Expression\nx=cluster, y=gene, z=scaled expression")
...
try({
  ...
  if (print.inline) print(plt.heat.all)
...
vst.merge.final$z.comp[(sum(is.infinite(vst.merge.final$z.comp)) | (vst.merge.final$z.comp > 10))] <- 10
