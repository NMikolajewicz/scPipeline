...
ensemble.parameters <- list(
  root.cluster.ensemble = "c1",
  tolerance.threshold = 0 # lineage extension, tolerance threshold (recommended range: 0 [stringent] to 0.15 [lenient])
)
...
if (do.paga){
  ...
  # py_run_string(paste0("r.alpha.uns['iroot'] = ", as.integer(root_cell)), local = F)
...
trajectory.Plot <- function(umap.x, umap.y, umap.cluster, edge.x1, edge.y1, edge.x2, edge.y2, edge.weights, 
  ...
  # clust <- as.numeric(df.umap$cluster
