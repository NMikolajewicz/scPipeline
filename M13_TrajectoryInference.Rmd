...
packages2load <- c("Seurat", "scater", "scMiko", "reticulate", "destiny", "foreach", "doParallel", "dtw", "clusterExperiment", 
                   ...
                   "slingshot", "tidymodels", "viridis", "future", "diffusr", "markovchain")
...
# Data subsampling
subsample_factor <- 0.1 # OPTIONAL; range 0 to 1 (default = 1)
...
```{r load data old, message=FALSE, warning=FALSE}
...
# reticulate::py_install("fa2", pip = TRUE)
...
if (do.paga){
  ...
  # run PAGA analysis
  pr <- PAGA(so.query,assay = DefaultAssay(so.query), seurat_grouping = "seurat_clusters", 
  ...
  # df.meta.py <- alpha$obs
  # df.meta.r <- py_to_r(df.meta.py)
  pseudotime <- pr@misc[["paga"]][["df.meta.r"]]$dpt_pseudotime
...
if (scPedigree.parameters$do.scPedigree){
...
bias.ensemble <- scPedigree.parameters$do.scPedigree
...
if (print.inline & !is.null(cor.res)) pheatmap::pheatmap(cor.res, main = "Correlation of Gene Importance Across Lineages")
...
for (i in 1:length(all.lineages)){
  ...
  # try({
  #   so.query@assays[[DefaultAssay(so.query)]]@data@Dimnames[[1]] <- so.query@assays[[DefaultAssay(so.query)]]@meta.features[["SYMBOL"]]   
  # }, silent = T)
...
temporalModuleSignatures <- function(df, time = df$time, module.list){
  ...
  for (j in 1:length(module.list)){
    ...
    # df.mod.signature <- dplyr::inner_join(df.mod.signature, new.df, by = "time")
    # df.mod.standarderror <- dplyr::inner_join(df.mod.standarderror, new.df.se, by = "time")
