...
knit: (function(inputFile, encoding) {
    rmarkdown::render(input = inputFile,
      encoding = encoding,
      output_file = paste0(
        xfun::sans_ext(inputFile), '_', format(Sys.Date(), "%d%m%y"),user, '.html'
      ),
...
run.id <- NULL
try({
run.id <-  updateCentralLog(Module = "M01", pdf.flag = save.pdf)
}, silent = T)
if (is.null(run.id)) run.id <- ""
...
testplots <- list(
  QC.violin = list(plot = plt.QC_violin, dim = c(10,5)),
  QC.scatter = list(plot = plt.QC_scatter, dim = c(12,5)),
  QC.mito = list(plot = plt.mito_by_bc + theme_miko(legend = T), dim = c(5,5))
)
somePDFPath = "test.pdf"
pdf(file=somePDFPath)  

for (i in 1:length(testplots)){

  print(testplots[[i]]$plot,
        width = testplots[[i]]$dim[1],
        height = testplots[[i]]$dim[2])
} 
dev.off() 
...
print(plt.QC_violin)
savePDF(file.name = "M01_QC_violin.pdf", plot.handle = plt.QC_violin, fig.width = 10, fig.height = 5, save.flag = save.pdf)
...
savePDF(file.name = "M01_QC_scatter.pdf", plot.handle = plt.QC_scatter, fig.width = 12, fig.height = 5, save.flag = save.pdf)
```
...
savePDF(file.name = "M01_mito_by_bc.pdf", plot.handle = plt.mito_by_bc + theme_miko(legend = T), fig.width = 8, fig.height = 5, save.flag = save.pdf)
```
...
savePDF(file.name = "M01_filter_pre_post.pdf", plot.handle = plt.filter_pre_post + theme_miko(legend = T), 
