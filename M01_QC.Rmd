...
analysis.parameters <- analysisParameters(
  which.data = c("Rambow.Melanoma"),
  # which.strata = c("LM.trouble"),
  organism.filter.flag = F,                             
  organism.include = c("Hs"),
  save.flag = T,                                                              # OPTIONAL; logical (default = T)  # save results
  save.filename = "Rambow_Melanoma_221120.Rdata", # note that prefix containing module number and run ID is appended to save file. 
  save.directory = "Preprocessed_Datasets/",
  cluster.resolution = 1,
  gene.upperlimit = 9000, #9000
  gene.lowerlimit = 200,  #200
  mt.upperlimit = 60,   #60
  # unmatched.rate.filter.flag = F,
...
if (length(which.data) == 1){
  analysis.parameters$vars2regress <- analysis.parameters$vars2regress[!(analysis.parameters$vars2regress %in% "dataset")]
}
...
try({
 analysis.parameters$organism.include <- rep(analysis.parameters$organism.include, length(input_data))
}, silent = T)
...
df.log <- addLogEntry("gene/cell Upper Limit (N/cell)", as.character(analysis.parameters$gene.upperlimit), df.log, "gene.upperlimit")
df.log <- addLogEntry("gene/cell Lower Limit (N/cell)", as.character(analysis.parameters$gene.lowerlimit), df.log, "gene.lowerlimit")

df.log <- addLogEntry("Filtered by unmatched reads", 
                      as.character(analysis.parameters$unmatched.rate.filter.flag), df.log, "unmatched.rate.filter.flag")

df.log <- addLogEntry("Mitochondrial Content Upper Limit (%/cell)", as.character(analysis.parameters$mt.upperlimit), df.log, "mt.upperlimit")
...
df.log <- addLogEntry("Cluster Resolution", as.character(analysis.parameters$cluster.resolution), df.log, "cluster.resolution")
df.log <- addLogEntry("Filtered by organism", as.character(analysis.parameters$organism.filter.flag), df.log, "organism.filter.flag")

if (analysis.parameters$organism.filter.flag) {
  df.log <- addLogEntry("Species Included", as.character(analysis.parameters$organism.include), df.log, "organism.include")
}
...
df.log <- addLogEntry("Variables regressed out", as.character(analysis.parameters$vars2regress), df.log, "vars2regress")
...
loadMat.dev <- function(import_set, input_organisms, dir) {
  
  import_set_path <- paste(dir, import_set, sep ="")
  
  # import expression matrix data
  
  which.set <- "rambow" # options: "kumal", "rambow"
...
for (i in 1:length(input_data)){
  ...
  if (input.type[i] == 1) {
    output.list <- loadMoffat(import_set = cur.total_sets, 
                                     subsample_factor = analysis.parameters$subsample_factor, 
                                     input_organisms = stringr::str_trim(unlist(strsplit(all_input_organisms[i], ",")))  , 
                                     organism_include = analysis.parameters$organism.include[i], 
                                     dir[i])
  } else if (input.type[i] == 2){
    output.list <- loadCellRanger(import_set = cur.total_sets, 
                                  stringr::str_trim(unlist(strsplit(all_input_organisms[i], ","))), 
                                  dir[i])
...
#                                error = function(e) {return(1)})
#   
#   # a <- getDensity(df.meta$nCount_RNA, df.meta$nFeature_RNA, n = 100)
# 
#   # construct plots
...
if (analysis.parameters$print.inline) print(plt.cluster_composition_barcodes)
...
if ("Phase" %in% names(so@meta.data)){
  ...
  plt.cc <- cowplot::plot_grid(plt.umap_by_cluster, 
                               plt.cc.umap + ggthemes::scale_colour_tableau(), 
                               plt.cc.compo + ggthemes::scale_fill_tableau(), ncol = 3)
} 
...
try({
  run.id <-  updateCentralLog(Module = "M01", input.data = which.data, input.subset = which.strata, pdf.flag = save.pdf)
...
analysis.parameters$save.filename <- paste0(run.id, "_", analysis.parameters$save.filename)

df.log <- addLogEntry("Output Results (.Rdata)", as.character(analysis.parameters$save.filename), df.log, "save.filename")
df.log <- addLogEntry("Output Directory", as.character(analysis.parameters$save.directory), df.log, "save.directory")
df.log <- addLogEntry("Output Saved", as.character(analysis.parameters$save.flag), df.log, "save.flag")

# save results to Preprocessed_Datasets/
save.filename <- paste0(data.path, analysis.parameters$save.directory , analysis.parameters$save.filename)
...
savePDF(file.name = paste0(output.path, "PDF/", "M01_QC_violin.pdf"), plot.handle = plt.QC_violin, fig.width = 10, fig.height = 5, save.flag = save.pdf)
...
savePDF(file.name = paste0(output.path, "PDF/", "M01_QC_scatter.pdf"), plot.handle = plt.QC_scatter, fig.width = 12, fig.height = 5, save.flag = save.pdf)
...
savePDF(file.name = paste0(output.path, "PDF/", "M01_mito_by_bc.pdf"), 
        plot.handle = plt.mito_by_bc + theme_miko(legend = T), fig.width = 8, fig.height = 5, save.flag = save.pdf)
...
savePDF(file.name = paste0(output.path, "PDF/", "M01_UMI_per_cell.pdf"), plot.handle = plt.UMI_per_cell + theme_miko(legend = T), 
        fig.width = 5, fig.height = 5, save.flag = save.pdf)
```
...
```{r plt QC ranks mT, fig.width = 15, fig.height = 4}
 cowplot::plot_grid(plt.gene.umi.mito, plt.umi.rank.mito, plt.gene.rank.mito, plt.mt.rank.mito, ncol = 4) 

savePDF(file.name = paste0(output.path, "PDF/", "M01_QC_mt_ranks.pdf"), 
        plot.handle =  cowplot::plot_grid(plt.gene.umi.mito, plt.umi.rank.mito, plt.gene.rank.mito, plt.mt.rank.mito, ncol = 4), 
...
try({
  plot(plt.filter.venn) 
  savePDF(file.name = paste0(output.path, "PDF/", "M01_venn_filter.pdf"), plot.handle =  plot(plt.filter.venn)  , 
          fig.width = 5, fig.height = 5, save.flag = save.pdf)
}, silent = T)
```
...
```{r plt.PCA, fig.width=8, fig.height = 4}

cowplot::plot_grid(plt.scree1, plt.scree2, ncol=2)
