...
# Rapid marker finding (logical flag) t
# quick_markers <- F 
...
if (run.deg){
  ...
  deg.results <- foreach(j = 1:nrow(all.pair.ordered.u), .packages = c("Seurat", "dplyr"), .errorhandling = 'remove')  %dopar% {
    ...
    deg.res <- NULL
      ...
      deg.res <- presto::wilcoxauc(so.query[ ,c(which.cells.i, which.cells.j)], group_by = "seurat_clusters", seurat_assay = DefaultAssay(so.query))
      deg.res$gene <- deg.res$feature
      deg.res <- deg.res %>% dplyr::filter(padj < p.threshold, auc > 0.5, logFC > lfc.threshold)
  ...
  deg.res.all$pct.dif <- deg.res.all$pct_in - deg.res.all$pct_out
  which.round <- c("avgExpr", "pval", "logFC","pct_in","pct_out","padj","pct.dif", "auc", "statistic")
...
message("Generating expression heatmaps...")
...
for (i in 1:length(all.pairs)){
  ...
  plt.pairwise.deg.map <- tryCatch({
        ggplotify::as.ggplot(pheatmap::pheatmap(emat.current, 
                                                                  show_colnames = F, 
