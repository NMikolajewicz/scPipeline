...
which.annotation.db <- "Bader"    # Reactome, GO, Bader, msigdb
...
# specify pairwise comparisons of interest (cluster id); set to NA for all comparisons
which.clusters <- c(0,1,2)
...
gsea.p.threshold <- 0.01
gsea.NES.threshold <- 1.5
...
deg.results <- list()
deg.results <- foreach(j = 1:nrow(all.pair.ordered.u), .packages = c("Seurat", "dplyr"))  %dopar% {
...
if (run.deg){
  ...
  # if na, set to 0 (no differentials found)
...
# filter clusters 
if (exists("which.clusters") && !is.na(which.clusters)){
  df.umap.centers <- df.umap.centers[df.umap.centers$cluster %in% which.clusters, ]
}
...
top.gsea.all <- gsea.path.all.nodup[seq(1,max.paths), ]
