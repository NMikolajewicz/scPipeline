...
parameter.list <- list(
  ...
  # input parameters
  input.object = "C:/Users/Owner/Dropbox/PDF Projects - JM/R Packages/scMiko/data/so_tyser2021_220621.rds",    # seurat object
  # input.object = NULL,           
  input.cell_ranger = NULL, #"C:/Users/Owner/Dropbox/PDF Projects - JM/Data/scRNA-seq/01_sci-RNA-seq3_Hong_Kevin_Jason/NM_HH/Data/Raw_Datasets/cell_ranger/15-Neftel_2019/",                            # gene count matrix
  input.matrix = NULL, #"C:/Users/Owner/Dropbox/PDF Projects - JM/Data/scRNA-seq/01_sci-RNA-seq3_Hong_Kevin_Jason/NM_HH/Data/Raw_Datasets/Expression_matrix/EMT6_invivo_CTLscreen_240320/overall_Matrix_240320.txt", # as csv file - first column has genes, rows are cell names
  # input.genes = NULL,
  # input.format = "cell_ranger",                   # options: matrix, cell_ranger

  
  # output parameters
  save.flag = F,              
  output.object.directory = "Preprocessed_Datasets/",
  output.object.file = "p16_mm_gbm_150122.Rdata",
...
# 
# 
# flex.asDT(data.frame(table(so@meta.data[["Barcode"]])))
...
# plt.handle <- VariableFeaturePlot(so, cols = c('#18BC9C','#2C3E50'))
...
# combine plots
plt.umap.combo <- cowplot::plot_grid(plt.umap_by_cluster, plt.umap, ncol = 2, labels = c("A", "B"))
...
if (is.null(plt.umap_umr)){
  plt.umap.qc <- cowplot::plot_grid(plt.umap_umi + theme(legend.position = "bottom"), 
                                    plt.umap_features + theme(legend.position = "bottom"), 
                                    plt.umap_mito + theme(legend.position = "bottom"), 
                                    ncol = 3, align = "hv", labels = c("C", "D", "E"))
} else {
  plt.umap.qc <- cowplot::plot_grid(plt.umap_umi + theme(legend.position = "bottom"), 
                                    plt.umap_features + theme(legend.position = "bottom"), 
                                    plt.umap_mito + theme(legend.position = "bottom"), 
                                    plt.umap_umr + theme(legend.position = "bottom"), 
