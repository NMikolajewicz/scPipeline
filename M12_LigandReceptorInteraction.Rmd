...
prep.data <- function(so, cluster.field, gNames.list, data.slot){
  ...
  # RESUME HERE
  # Construct matrix in chunks

  
  exp.df_t <- tryCatch({
    exp.df_t <- t(as.data.frame(exp.mat))
  }, error = function(e){
    # print(e)
    exp.df_t <- NULL
    chunk.size <- 10000
    n.blocks <- round(ncol(exp.mat) / chunk.size)+1
    for (i in 1:(n.blocks)){
      start.range <- ((i-1)*chunk.size)+1
      end.range <- i*chunk.size
      if (start.range > ncol(exp.mat)) next
      if (end.range > ncol(exp.mat)) end.range <- ncol(exp.mat)
     exp.df_t <- bind_rows(exp.df_t,as.data.frame(t(as.matrix(exp.mat[, start.range:end.range]))))
    }
    return(exp.df_t)
  })
...
# options: #cytokine, checkpoint, growth factor, pooled, other , "pooled")
