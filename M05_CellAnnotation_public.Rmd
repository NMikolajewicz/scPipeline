...
parameter.list <- list(
  # input.file = "R646_M02_NM2_p14_Meso_FZDonlg_1000g_integrated_rpca_280921.Rdata",
  ...
  # which.references = c( "PijuanSala2019"),
  external.markers = NULL,
  internal.markers = "Cell_Catalog",
  marker.filter = "Manno|Tyser", # filter marker sets using `grepl` pattern matching. E.g., Manno2021 for all La Manno 2021 atlas-derived markers. 
  coherence.threshold = 0.8,
  p.threshold = 0.05,
  frequent.filter.threshold = 0.75,
  filter.frequent.fliers = F,
  
  min.representation = 0.5, # if fraction of gene present in seurat data is less than in gene set, gene set is omitted form conisderation

  cluster.resolution = 0.6, # OPTIONAL; range 0 to inf (default = 1)
  fdr.correction = F, 
  clean.clusters = F,
  max.cells = 20000,
...
miko_message("Generating annotation wordclouds...")
...
savePDF(file.name = paste0(output.path, "PDF/", "M05_null_enrichment_model.pdf"), plot.handle = plot.null.enrichment,
...
for (i in 1:length(plt.ww.list)){
  ...
  plot.name <- paste0("M05_wordcloud_", cur.plot.name ,".pdf")
