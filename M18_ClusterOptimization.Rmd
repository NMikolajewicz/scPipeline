...
cluster.tree.markers <- c("CD34" , "Sox2")
...
# free up some space
try({rm(so.query_small)}, silent = T)
...
#
meta.clust.entries <- paste0(DefaultAssay(so.query), "_snn_res.", cluster_resolution)
df.meta <- so.query@meta.data
df.meta <- df.meta[, meta.cols %in% meta.clust.entries]
df.meta[ ,paste0(DefaultAssay(so.query), "_snn_res.", 0)] <- 0
df.meta$umap.x <- so.query@reductions[["umap"]]@cell.embeddings[ ,1]
df.meta$umap.y <- so.query@reductions[["umap"]]@cell.embeddings[ ,2]

so.query@meta.data[[paste0(DefaultAssay(so.query), "_snn_res.", 0)]] <- 0

# plot cluster trees
plt.clusttree <- clustree(so.query, prefix =  paste0(DefaultAssay(so.query), "_snn_res."))

if (print.inline) print(plt.clusttree)

plt.umap.tree <- clustree_overlay(df.meta, prefix =  paste0(DefaultAssay(so.query), "_snn_res."), 
...
if (!is.na(cluster.tree.markers)){
  for (i in 1:length(cluster.tree.markers)){
    ...
    try({
    plt.clusttree.markers[[cluster.tree.markers[i]]] <- clustree(so.query, prefix =  paste0(DefaultAssay(so.query), "_snn_res."), 
