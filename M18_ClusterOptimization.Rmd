...
parameter.list <- list(
  ...
  # input.file = "R579_M01_NM2_p15_HAP1_GIN_230821.Rdata",
  input.file = "R581_M02_NM2_p15_HAP1_GIN_integrated_rpca_230821.Rdata",
  ...
  cluster.resolution = c(0.05, 0.1, 0.2, 0.3,0.35, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1),
  lfc.threshold = 0.5, # log fold change threshold (0.5 recommended)
  fdr.threshold = 0.05,
  group.singletons = F, # Default: T (recommended for clean datasets)
...
cluster.membership <- foreach(i = 1:length(parameter.list$cluster.resolution), .packages = c("Seurat"))  %dopar% {
  so.query <- FindClusters(object = so.query, resolution = parameter.list$cluster.resolution[i], 
                           group.singletons = parameter.list$group.singletons,
...
if (ncol(so.query) > 10000){
  so.sub <- so.query[ , sample(colnames(so.query), 10000)]
  df.pc <- propVarPCA(so.sub)
  so.sub <- FindNeighbors(so.sub, dims = 1:(sum(df.pc$pc.cum_sum < 0.9) + 1), nn.method = "rann")
}
for (i in 1:length(parameter.list$cluster.resolution)) {
  current.cluster <- paste0(assay.pattern, parameter.list$cluster.resolution[i])
  if (ncol(so.query) > 10000){
    
    so.sub <- neighborPurity(object = so.sub, graph = cluster.graph, cluster.field = current.cluster) 
...
plt.cluster.purity <- df.purity.sum %>%
  ...
  theme_miko() + 
  labs(x = "Resolution", y = "Purity Score", title = "Purity Score") + 
...
plt.clust.spec <- qm.res.sum.sum.all %>% 
  ...
  theme_miko(legend = T) + 
  xlab("AUC") + 
  ylab("Proportion of Clusters (>=1 DEG exceeding target DEG-AUC)") + 
  geom_vline(xintercept = 0.5, linetype = "dashed") + 
  labs(title = "Cluster Marker Specificity", subtitle = "Proportion of clusters with atleast one DEG exceeding target DEG-AUC",
...
plt.auc.spec <- df.auc.spec %>%
  ...
  # geom_line(data = df.silw.sum, aes(x = cluster.resolution, y = sil.mean, group = 1)) + 
  theme_miko() + 
  labs(x = "Resolution", y = "Marker Specificity Score", title = "Marker Specificity") + 
  theme(panel.grid.minor = element_line(colour="grey95", size=0.1),
        panel.grid.major = element_line(colour="grey85", size=0.1),
        panel.grid.minor.y = element_blank(),
        panel.grid.major.y = element_blank()) +
    scale_x_continuous(minor_breaks = seq(0 , max(df.auc.spec$res, na.rm = T), 0.1) , 
                       breaks = seq(0, max(df.auc.spec$res, na.rm = T), 0.2))
...
if (exists("so.sub")){
  df.umap <- getUMAP(so.sub)[["df.umap"]]
...
for (i in 1:length(all.deg.list)){
  ...
  sil <- cluster::silhouette(
    x = as.numeric(as.character(so.sub@meta.data[,set.name] )),
    dist = umap.dist)
...
plt.opt.stat <- cowplot::plot_grid( plt.silw.dep + labs(subtitle = "1 = dense/separated clusters; 0 = overlapping clusters") , 
                                    plt.cluster.purity + labs(subtitle = "1 = within-neighborhood clusters consistent (pure); 0 = impure"), 
                                    plt.auc.spec + labs(subtitle = "0.5 = specific DEG in all clusters; 0 = no specific DEGs"),
