...
packages2load <- c("Seurat", "scMiko", "DT", "scales", "plyr", "miloR", "igraph",
...
parameter.list <- list(
  ...
  # input.file = "so_immune_PR_invivo_integrated_210421.rds",
  ...
  #                       p11 = c("B2m_MHC1neg.sc", "B2m_MHC1pos.sc"),
  #                       p12 = c("B2m_MHC1neg.sc", "B2m_MHC1neg.ortho"),
  #                       p13 = c("B2m_MHC1neg.ortho", "B2m_MHC1pos.ortho"),
  #                       p14 = c("B2m_MHC1pos.sc", "B2m_MHC1pos.ortho")),
  # da.control = "WT.sc", 
  da.which.pairs = list(P1 = c("C15", "C50"), 
                        P2 = c("C15", "WT"),
                        P3 = c("C15", "C76"),
                        P4 = c("C15", "C68"),
                        P5 = c("C15", "C2"),
                        ...
                        P10 = c("WT", "C68"),
                        P11 = c("WT", "C2"),
                        P12 = c("WT", "C53")),   # specify specific pairs to compare; list (e.g., list(pair1 = c("wt", "c1"), pair2 = c("wt", "c2"))
  # da.control = NA       # specify control group to compare all others pairs to

  table.output.n = 100, # output top N results in tables (per group)
  print.inline = FALSE ,
  save.pdf = T,
  update.log = T
...
if ("save.pdf" %in% names(parameter.list)){
  save.pdf <- parameter.list$save.pdf
} else {
  save.pdf <- T
}

if ("print.inline" %in% names(parameter.list)){
  print.inline <- parameter.list$print.inline
} else {
  print.inline <- F
}

if ("update.log" %in% names(parameter.list)){
  update.log <- parameter.list$update.log
} else {
  update.log <- T
}
...
try({
  df.bc.tally.final <- df.bc.tally
colnames(df.bc.tally.final) <- c("Barcode", "Cluster", "count", "proportion")
}, silent = T)
...
da.res.list.sc <- list()
da.comp.list <- list()
da.par <- c("da.which.pairs", "da.control")
da.par.av <- da.par[da.par %in% names(parameter.list)]

if (("da.control" %in% da.par.av) && !is.na(parameter.list$da.control)){
  ref.group <- parameter.list$da.control
...
if (("da.which.pairs" %in% da.par.av) && !is.na(parameter.list$da.which.pairs)){
  ...
  for (i in 1:length(pair.list)){
    ...
    if (n.pair > 2) message(names(pair.list)[i], " specifies more than 2 groups for case. Only first 2 groups used.")
...
# set seed prior to subsampling during DA analysis
set.seed(1023)

miko_message("Performing differential abudnance analysis (modified Milo method)...")
da.res.list <- list()
for (i in 1:length(da.comp.list)){

  miko_message(paste0(names(da.comp.list)[i], " is being evaluated..."))
  so.query@meta.data$da.group <- NA
  so.query@meta.data$da.group[so.query@meta.data$bc2 %in% da.comp.list[[i]][["control"]]] <- "control"
  so.query@meta.data$da.group[so.query@meta.data$bc2 %in% da.comp.list[[i]][["case"]]] <- "case"
  ...
  min.size <- min(df.group.tally$Freq)
    ...
    res.current <- da_Run(object = so.query[ , !is.na(so.query@meta.data$da.group)], 
                          ...
                          balance.size = min.size, reference.group = "control")
...
for (i in 1:length(da.res.list)){
  plt.da.list[[names(da.res.list)[i]]] <- da.res.list[[i]]$da.umap +
    labs(title = names(da.res.list)[i],
         caption = paste0("Control: " , paste(da.comp.list[[i]][["control"]], collapse = ", "), 
                           "\nCase: " , paste(da.comp.list[[i]][["case"]], collapse = ", ")))
}
...
da.umap.list[[names(da.res.list)[i]]] <- da_UMAP(da.res.list = da.res.list[[i]], color.by = "logFC", sig.only = sig.da, 
                             xlim.cart= c(p13.xmin,p13.xmax), ylim.cart= c(p13.ymin,p13.ymax), scale.lim = 8) + 
  labs(title =names(da.res.list)[i])
...
getIntraClusterDEG <- function(res.list.list){
  ...
  for (k in 1:length(u.clust)){
    ...
    deg.geno.sig.up <- deg.geno %>% dplyr::filter(!grepl("control", group), padj < 0.05, logFC > 0)
     deg.geno.sig.down <- deg.geno %>% dplyr::filter(!grepl("control", group), padj < 0.05, logFC < 0)
...
for (i in 1: length(u.groups)){
 ...
 # store deg results
  ...
  plt.volc <- current.deg %>%
    ...
    ggrepel::geom_text_repel(data = current.deg.top, aes(x = logFC, y = logp, label = feature), max.overlaps = Inf)
...
for (i in 1:length(da.comp.list)){
  ...
  group.n <- length(da.comp.list[[i]][["control"]]) + length(da.comp.list[[i]][["case"]])
  
  ref <- stringr::str_wrap(paste(da.comp.list[[i]][["control"]], collapse = ", "), 20)
  comp <- stringr::str_wrap(paste(da.comp.list[[i]][["case"]], collapse = ", "), 20)
  df.ref.comp <- data.frame(Group = c("Control", "Case"), x = c(ref, comp))
  colnames(df.ref.comp) <- c("Pair", parameter.list$da.group.by)
  
  df.ann.list[[pair.name]] <- df.ref.comp
  
  ref <- paste(da.comp.list[[i]][["control"]], collapse = ", ")
  comp <- paste(da.comp.list[[i]][["case"]], collapse = ", ")
  df.ref.comp <- data.frame(Group = c("Control", "Case"), x = c(ref, comp))
  colnames(df.ref.comp) <- c("Pair", parameter.list$da.group.by)
...
for (i in 1:length(da.res.list)){
  ...
  # bonferroni-corrected 95% z score threshold
  n.clust <- ulength(df.res$seurat_clusters)
  bonf.z.score <- qnorm(1-((0.05 /n.clust)/2))
  ...
  df.res.sum <- df.res %>%
    ...
    dplyr::summarize(x.mean = mean(logFC, na.rm = T),
                     ...
                     x.se = sd(logFC, na.rm = T)/sqrt(n.clust),
                     x.ci = bonf.z.score * sd(logFC, na.rm = T)/sqrt(n.clust),
                     n.neighborhoods = length(logFC))
  
  df.res.sum$x.ci.low <- df.res.sum$x.mean - df.res.sum$x.ci
  df.res.sum$x.ci.high <- df.res.sum$x.mean + df.res.sum$x.ci
  
  df.res.sum$z <- df.res.sum$x.mean / df.res.sum$x.se
  df.res.sum$pval <- (2*pnorm(-abs(df.res.sum$z))) * n.clust
  df.res.sum$pval[df.res.sum$pval > 1] <- 1
  df.res.sum$is.sig <- abs(df.res.sum$z) > bonf.z.score
  
  df.res.sum.out <- df.res.sum
  df.res.sum.out$pair <- pair.name
  df.da.summary <- bind_rows(df.da.summary, df.res.sum.out)
  
  
  df.res.sum2 <- df.res.sum %>% dplyr::filter(abs(z)> bonf.z.score)
  ...
  df.res$z <- df.res$logFC /   sd(df.res$logFC, na.rm = T)
    ...
    plt.da.stat <- df.res.sum %>%
    ggplot(aes(x = reorder(seurat_clusters, x.mean), y = x.mean, color = (is.sig))) + 
      geom_point() + 
      geom_segment(aes(y = x.ci.low , yend = x.ci.high, x = seurat_clusters, xend = seurat_clusters)) + 
      scale_color_manual(values = c("TRUE" = "tomato", "FALSE" = "black")) + 
    geom_hline(yintercept = c(0), linetype = "dashed") +
    theme_miko(center.title = T, grid = T) + 
    labs(x = "Cluster",  y = "Differential Abundance (logFC +/- 95% CI)\nCase - Control", title = paste0(pair.name, " Differential Abundance"), subtitle = "Stratified by cluster (p<0.05 in red)", caption = "dashed lines = 0 (i.e., same abundance)") + 
    coord_flip()
  ...
  plt.da.umap <- da.umap.list[[pair.name]] +
      theme(plot.title = element_text(hjust = 0.5)) +
      theme(plot.subtitle = element_text(hjust = 0.5)) + 
    labs(title = "Differential Abundance UMAP", subtitle =  "Case - Control") + 
    theme(panel.border = element_rect(colour = "black", fill=NA))
  ...
  plt.summary.list[[pair.name]] <- cowplot::plot_grid( p2, plt.umap, plt.da.umap, plt.da.stat, nrow = 1, labels = "AUTO", align = "h", axis = "tb")
...
```{r under and overrepresented population gene associations, fig.width=18, fig.height=10}
...
for (i in 1:length(da.deg.res)){
  ...
  if (length(neg.group) > 0){
    ...
    plt.da.neg.all <- dge.neg.cur2 %>%
      ...
      scale_color_manual(values = c("TRUE" = "black", "FALSE" = "grey"), guide = F)+
            theme_void() + 
      labs(title = "Underrepresented populations", subtitle = "Case < Control (abundance)", 
           size = "H Size", fill = "Differential\nAbundance\n(logFC)",
           caption = "logFC = Case - Control") +
      theme(plot.title = element_text(hjust = 0.5)) +
      theme(plot.subtitle = element_text(hjust = 0.5)) + 
       scale_alpha_continuous(guide = FALSE) +
      geom_point(shape = 21)  + theme(panel.border = element_rect(colour = "black", fill = NA))
  } else {
    plt.da.neg.all <- NULL
  }
  ...
  if (length(pos.group) > 0){
    ...
    plt.da.pos.all <- dge.pos.cur2 %>%
      ...
      scale_color_manual(values = c("TRUE" = "black", "FALSE" = "grey"), guide = F)+
            theme_void() + 
            labs(title = "Overrepresented populations", subtitle = "Case > Control (abundance)", 
                 size = "H Size", fill = "Differential\nAbundance\n(logFC)",
           caption = "logFC = Case - Control") +
      theme(plot.title = element_text(hjust = 0.5)) +
      theme(plot.subtitle = element_text(hjust = 0.5)) + 
  ...
  n.up.ab <- sum(colnames(da.res.list[[pair.name]][["seurat.object"]]) %in% pos.cell2)
  n.down.ab <- sum(colnames(da.res.list[[pair.name]][["seurat.object"]]) %in% neg.cell2)
  ...
  if (n.up.ab > 0){
    ...
    common.up.deg  <- presto::wilcoxauc(X = da.res.list[[pair.name]][["seurat.object"]], group_by = "up.ab", 
                                        seurat_assay = DefaultAssay(da.res.list[[pair.name]][["seurat.object"]])) %>% dplyr::filter(group == "TRUE")
    ...
    common.up.deg <- common.up.deg %>% dplyr::filter(group == "TRUE", padj < 0.05)
    common.up.gsea$plt.gsea <- common.up.gsea$plt.gsea +  
      labs(subtitle = "Pathways associated with overrepresented populations") + 
    ...
    df.gsea.up$pair <- df.deg.up$pair <- pair.name
    ...
    plt.volc.common.up <- common.up.deg %>%
      ...
      geom_point() +
      ggrepel::geom_text_repel(data = common.up.deg.show, aes(x = logFC, y = -log10(padj), label = feature), max.overlaps = Inf) + 
      ...
      labs(x = "Expression logFC\n(overrepresented neighborhoods - rest)", y = "-log10(FDR)", title = "Volcano plot", "Differential Expression", caption = "horizontal dashed line = 5% FDR", subtitle = "Genes associated with overrepresented populations")
        ...
        if (!is.success) common.up.deg <- common.up.gsea <- df.gsea.up <- df.deg.up <-  common.up.deg.top <- common.up.deg.bottom <- plt.volc.common.up <- NULL
  } else {
    common.up.deg <- common.up.gsea <- df.gsea.up <- df.deg.up <-  common.up.deg.top <- common.up.deg.bottom <- plt.volc.common.up <- NULL
  }
  ...
  if (n.down.ab > 0){
    ...
    common.down.gsea <- runGSEA(gene = common.down.deg$feature, value = common.down.deg$logFC, 
                                species = parameter.list$species, db = parameter.list$pathway.db, plot.top.n = 7)
    common.down.deg <- common.down.deg %>% 
      dplyr::filter(group == "TRUE", padj < 0.05)
    common.down.gsea$plt.gsea <- common.down.gsea$plt.gsea +  
      labs(subtitle = "Pathways associated with underrepresented populations")  + 
      theme_miko(center.title = T, legend = T) +
      theme(axis.text.y = element_text(size=6))
    ...
    df.deg.down$pair <- df.gsea.down$pair <- pair.name
    ...
    plt.volc.common.down <- common.down.deg %>%
      ggplot(aes(x = logFC, y = -log10(padj))) + 
      geom_point() +
      ggrepel::geom_text_repel(data = common.down.deg.show, aes(x = logFC, y = -log10(padj), label = feature), max.overlaps = Inf) + 
      theme_miko(center.title = T) + geom_vline(xintercept = 0, linetype = "dashed") + geom_hline(yintercept = -log10(0.05), linetype = "dashed") + 
    ...
    if (!is.success){
      common.down.deg <- common.down.gsea <- df.gsea.down <- df.deg.down <- common.down.deg.top <- common.down.deg.bottom <- plt.volc.common.down <- NULL
    }
  } else {
    common.down.deg <- common.down.gsea <- df.gsea.down <- df.deg.down <- common.down.deg.top <- common.down.deg.bottom <- plt.volc.common.down <- NULL
  }
  ...
  # combine plots
    p1 <- cowplot::plot_grid(
     ...
     nrow = 2, rel_widths = c(1,1,1.5), align = "hv", axis = "tb", labels = "AUTO")
...
```{r abundance table output}

flex.asDT(df.bc.tally.final)
...
**Definitions**:\
**Pair (P)**: Two groups of samples representing different conditions. Groups are designated 'Case' and 'Control' for analyses.\
...
**Figure Legends**:\
**A| Pair specification.** Specification of samples belonging to 'Control' and 'Case' groups.\
**B| Cell cluster UMAP.** UMAP of samples belonging to 'Control' and 'Case' groups. *Color*: louvain cluster membership.\
**C| Differential abundance UMAP.** *Nodes*: neighborhoods, *colors*: logFC between Case and Control group, *size*: number of cells within neighborhood.\
**D| Differential abundance by cluster.** Cell clusters correspond to those shown in *B*.\
...
```{r save regional abundance abundance plots,include = FALSE}

try({
  for (i in 1:length(plt.summary.list)){
    plot.name <- paste0("DA_summary_", names(plt.summary.list)[i] ,".pdf")
...
```{r save abundance table 2}

colnames(df.da.summary) <- c("cluster", "logFC", "sd", "se", "ci.width", "n.neigborhoods", "ci.low", "ci.low", "z", "p", "significant", "pair")
df.da.summary <- df.da.summary %>% dplyr::select(c("pair", "cluster", "logFC", "sd", "se","n.neigborhoods",  "ci.width", "ci.low", "ci.low", "z", "p", "significant"))

round.features <- c("logFC", "sd", "se", "ci.width", "ci.low", "ci.low", "z", "p")
for (i in 1:length(round.features)){
  df.da.summary[ ,round.features[i]] <- signif(df.da.summary[ ,round.features[i]], 3)
}


flex.asDT(df.da.summary)
```
...
```{r save hood-specific abundance plots,include = FALSE}

try({
  for (i in 1:length(da.deg.final)){
    plot.name <- paste0("neighborhood_specific_DA_", names(da.deg.final)[i] ,".pdf")
    plot.name <- gsub(" -> ", "-", plot.name)
    plot.name <- gsub(":", "-", plot.name)
    plot.name <- gsub(" ", "", plot.name)
    savePDF(file.name = paste0(output.path, "PDF/", plot.name), 
            plot.handle = da.deg.final[[i]], 
            fig.width = 20, fig.height=5, save.flag = parameter.list$save.pdf)
  } 
}, silent = T)
...
**Pair (P)**: Two groups of samples representing different conditions. Groups are designated 'Case' and 'Control' for analyses.\
**Neighborhood (H)**: Group of 10-50 similar cells identified on KNN graph.\
**Overrepresented population**: Neighborhoods in which abundance in 'Case' group is greater than in 'Control' group.\
**Underrepresented population**: Neighborhoods in which abundance in 'Case' group is less than in 'Control' group. 
...
**B| Volcano plot** of differential expressed genes between overrepresented population and rest of cells. LogFC > 0 and LogFC < 0 represent over- and under-expressed genes, respectively, in overrepresented population (Case > Control abundance)\
...
**E| Volcano plot** of differential expressed genes between underrepresented population and rest of cells. LogFC > 0 and LogFC < 0 represent over- and under-expressed genes, respectively, in underrepresented population (Case < Control abundance).\
...
```{r save population-wide abundance plots,include = FALSE}

try({
  for (i in 1:length(plt.common.deg.all)){
    plot.name <- paste0("population_wide_DA_", names(plt.common.deg.all)[i] ,".pdf")
    plot.name <- gsub(" -> ", "-", plot.name)
    plot.name <- gsub(":", "-", plot.name)
    plot.name <- gsub(" ", "", plot.name)
    savePDF(file.name = paste0(output.path, "PDF/", plot.name), 
            plot.handle = plt.common.deg.all[[i]], 
            fig.width = 18, fig.height=10, save.flag = parameter.list$save.pdf)
  } 
}, silent = T)
...
deg.list.all$group[deg.list.all$group %in% "overrepresented"] <- "case group"
deg.list.all$group[deg.list.all$group %in% "underrepresented"] <- "Control group"
...
gsea.list.all$group[gsea.list.all$group %in% "overrepresented"] <- "case group"
gsea.list.all$group[gsea.list.all$group %in% "underrepresented"] <- "Control group"
