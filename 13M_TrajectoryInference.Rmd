---
title: "Module13_trajectory_inference"
output: 
...
packages2load <- c("Seurat", "scater", 
                   ...
                   "slingshot", "tidymodels", "viridis", "future")
...
# Query input
# input.file <- "Module2_integrated_p3_p5_differentiation_090120.Rdata"
...
# UHN
# scPedigree.parameters <- list(
#   source = c(2,0),           # cluster ids ordered by pseudotime trajectory. upstream trajectory for all branches
#   branch = list(                 # cluster ids ordered by pseudotime trajectores, lineage specific trajectories. 
#     branch.1 = c(12),
#     branch.2 = c(11),
#     branch.3 = c(9),
...
if (do.paga){
  ...
  u.clust <- as.numeric(unique(as.character(so.query@active.ident)))
  
  
  # run PAGA analysis
  pr <- PAGA(so.query,assay = "RNA", seurat_grouping = "seurat_clusters", 
  ...
  paga.plot <- ggplot() +   
    ...
    labs(x = "UMAP 1", y = "UMAP 2", title = "UMAP", subtitle = "PAGA Trajectories") +
    theme_bw() +
    theme(
...
if (!is.null(start.cluster)){
  ...
  # tip.cluster <- start.cluster
} else {
  # tip.cluster <- as.numeric(as.character(df.dm$cluster)[which.min(df.dm$pseudotime_diffusionmap)])
...
# tip.cluster <- as.character(df.dm$cluster)[which.min(df.dm$pseudotime_diffusionmap)] 
...
plt.dmap.peudotime <- ggplot(df.dpt) + geom_point(aes(x = DC1, y = DC2, color = dptval)) + scale_color_viridis(name = "Pseudotime")
...
plt.dmap.cl <- ggplot(df.tmp, aes(x = DC1, y = DC2, colour = cluster)) +
  geom_point() + 
  ggbeeswarm::geom_quasirandom(groupOnX = FALSE) +
  xlab("Diffusion component 1") + 
  ylab("Diffusion component 2") +
...
dm.plot <- ggplot() +   
  ...
  geom_segment(
    data = df.dm.edge.sub,
    aes(x = x1, y = y1, xend = x2, yend = y2),
    size = df.dm.edge.sub$weight*7,
    colour = "black",
    show.legend = FALSE
  ) +
  geom_point(data = df.positions, aes(x, y, fill = group), size = 7, alpha = 1 , color = "black", pch=21, show.legend = FALSE) +
  geom_text(data = df.positions, aes(x, y, label = group), color = "black", fontface = "bold") +
  labs(x = "UMAP 1", y = "UMAP 2", title = "UMAP", subtitle = "Diffusion Map Trajectories") +
  theme_bw() +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()
...
# dpt_flat <- branch_divide(dpt, div)
# u.branch <- unique(dpt_flat@branch[, 1])
# u.branch <- u.branch[!is.na(u.branch)]
# root <- random_root(dpt_flat)
root <- tip.cluster
# root <- 4L
...
if (use.deg.reduction){
  ...
  # so.query <- FindClusters(object = so.query, resolution = cluster.resolution, verbose = 0, algorithm = 1)
}
...
sling <- slingshot(Embeddings(so.query, which.reduction)[ ,c(1:n.dim)], 
                   clusterLabels = as.vector(so.query@meta.data[[cur.cluster.field]]), 
...
df.red$cluster <- as.vector(so.query@meta.data[[cur.cluster.field]])
...
df.alt.red$cluster <- as.vector(so.query@meta.data[[cur.cluster.field]])
...
get.linkage.coordinates <- function(df.center, df.adjacency, sling){
  ...
  # u.cluster <- unique(df.center$cluster)
...
# create plots
plt.lineage <- ggplot() +
  geom_point(data = df.red, aes(x= x, y= y, color = cluster)) +
  theme_classic() +
  xlab(xlab.lineage) +
  ylab(ylab.lineage)  +
...
plt.lineage.alt <- ggplot() +
  geom_point(data = df.alt.red, aes(x= x, y= y, color = cluster)) +
  theme_classic() +
...
e.mat <- t(as.matrix(so.query@assays$SCT@data))
pc.mat <- (as.matrix(so.query@reductions[["pca"]]@cell.embeddings))
# e.mat <- t(as.matrix(so.query@assays[["SCT"]]@scale.data))
...
#   space,
#   progression_group =  factor(df.meta$Barcode),
#   path = traj$path,
#   contour = TRUE) +
#   theme_miko()
...
if ("fork" %in% df.sp.umap.centers$node.type){
  df.sp.umap.centers$node.type <- factor(df.sp.umap.centers$node.type, levels = c("end", "link", "start", "fork"))
}

plt.trajectory.sp <- ggplot() +
  geom_point(data = df.sp.umap, aes(x= x, y= y, color = cluster)) +
  theme_classic() +
  xlab("UMAP 1") +
  ylab("UMAP 2")  +
  geom_segment(data = df.sp.adj, aes(x = x.origin, y = y.origin, xend = x.destination, yend = y.destination), size = 1) +
  geom_point(data = df.sp.umap.centers, aes(x = x.center, y = y.center, fill = node.type ), pch = 21, size = 4) +
  ggtitle(paste("Lineages (", toupper(alt.reduction), ")", sep = "")) 
...
so.tempora <- ImportSeuratObject(so.query, 
                                 ...
                                 timepoint_order = tempora.timepoints$new.label)
...
trajectory.Plot <- function(umap.x, umap.y, umap.cluster, edge.x1, edge.y1, edge.x2, edge.y2, edge.weights, node.x, node.y, node.cluster){
  ...
  plot.trajectory <- ggplot() +   
    ...
    geom_point(data = df.centers, aes(x.center, y.center, fill = cluster), size = 7, alpha = 1 , color = "black", pch=21, show.legend = FALSE) +
    geom_text(data = df.centers, aes(x.center, y.center, label = cluster), color = "black", fontface = "bold") +
...
df.adj.all <-df.adj.paga[ ,c("origin", "destination")]
...
list.trajectories <- list(
  scPedigree = df.sp.adj.traj,
  slingshot = df.ss.adj.traj,
  dpt = df.dm.adj.traj,
  paga = df.adj.paga,
  tempora = df.tempora.adj.traj
)
...
df.ensemble.edges <- data.frame(origin = df.adj.paga$origin,
                                destination = df.adj.paga$destination,
                                x.origin = NA,
                                y.origin = NA,
                                x.destination = NA,
                                y.destination = NA)
...
ensemble.geom.plot <- trajectory.Plot(umap.x = df.umap$x, umap.y = df.umap$y, umap.cluster = df.umap$cluster, 
                                      edge.x1 = df.ensemble.edges$x.origin, edge.y1 = df.ensemble.edges$y.origin, 
                                      edge.x2 = df.ensemble.edges$x.destination, edge.y2 = df.ensemble.edges$y.destination, 
                                      edge.weights = df.ensemble.edges$geom.weights, 
                                      node.x = df.umap.centers$x.center, node.y = df.umap.centers$y.center, 
                                      node.cluster = df.umap.centers$cluster) + 
  labs(subtitle = "Ensemble: Geometric Mean (all)")

ensemble.mean.plot <- trajectory.Plot(umap.x = df.umap$x, umap.y = df.umap$y, umap.cluster = df.umap$cluster, 
                                      edge.x1 = df.ensemble.edges$x.origin, edge.y1 = df.ensemble.edges$y.origin, 
                                      edge.x2 = df.ensemble.edges$x.destination, edge.y2 = df.ensemble.edges$y.destination, 
...
ensemble.z.plot <- trajectory.Plot(umap.x = df.umap$x, umap.y = df.umap$y, umap.cluster = df.umap$cluster, 
                                   edge.x1 = df.ensemble.edges$x.origin, edge.y1 = df.ensemble.edges$y.origin, 
                                   edge.x2 = df.ensemble.edges$x.destination, edge.y2 = df.ensemble.edges$y.destination, 
...
# a.cluster <- as.numeric(as.character(so.query@meta.data[["seurat_clusters"]]))
# a.cluster <- data.frame(cluster = a.cluster[(a.cluster %in% which.root)])
# clust.tally <- as.vector( table(a.cluster))
...
plt.diffusion.simulation <- df.ht.long %>%
  ggplot(aes(x = time.point, y = heat, color = cluster)) + 
  geom_line(size = 1) + 
  theme_miko(legend = T) + 
  geom_hline(yintercept = classification.threshold, linetype = "dashed") + 
  ylab("State Frequency") + 
  xlab("Simulation Time") + 
  labs(title = "Ensemble Network Diffusion", subtitle = "Initial condition: Equally-distributed states")
...
plt.trajectory.ens <- ggplot() +
  geom_point(data = df.ens.umap, aes(x= x, y= y, color = cluster)) +
  theme_classic() +
  xlab("UMAP 1") +
  ylab("UMAP 2")  +
...
plt.ens.overview <- cowplot::plot_grid(plt.trajectory.ens, plot.lineage.umap.ens, rel_widths = c(2,3))
if (print.inline) {
  plt.ens.overview
}
...
top_hvg <- tryCatch({
  ...
  top_hvg <- HVFInfo(so.query) %>% 
    ...
    top_n(top.n.var, residual_variance) %>% 
    pull(bc)
  return(top_hvg)
})
...
if (print.inline) {
  ...
  # df.gene.importance.sub <- df.gene.importance
  # df.gene.importance.sub <- df.gene.importance[,grepl("Pedigree|gene", colnames(df.gene.importance))]
  
  
  # datatable(df.gene.importance.sub)
}
...
if (print.inline) pheatmap::pheatmap(cor.res, main = "Correlation of Gene Importance Across Lineages")
...
df.top.module.genes <- df.imp.module %>%
  group_by(module) %>%
  top_n(10, within_module_ordering)
...
if (sum(grepl("global", names(pt.list))) < 2){
  ...
  # if (print.inline) pheatmap::pheatmap(mat.glob.pt, main = "Global Pseudotime Correlations")
...
for (i in 1:length(all.lineages)){
  ...
  df.cor.pt <- bind_rows(df.cor.pt, data.frame(lineage = all.lineages[i], genes = colnames(pt.cor), rho = signif(pt.cor[1,], 3)))
...
for (i in 1:length(all.lineages)){
  ...
  so.query@assays[["RNA"]]@data@Dimnames[[1]] <- so.query@assays[["RNA"]]@meta.features[["SYMBOL"]]
  cur.data <- so.query@assays[["SCT"]]@data
  # cur.data <- so.query@assays[["SCT"]]@scale.data
  ...
  pt.cor <- cor(df.order$pseudotime, ph.mat, method = "spearman")
  df.gene.cor <- bind_rows(df.gene.cor, data.frame(genes = colnames(pt.cor), rho = signif(pt.cor[ 1,], 3), lineage = all.lineages[i]))
...
# get expression matrix
e.mat <- getExpressionMatrix(so.query, which.assay = "RNA", which.data = "data")
...
lin.ts.results <- (foreach(i = 1:length(u.lineage))) %dopar% {
  ...
  for (j in 1:length(genes.ts)){
    ...
    try({
      ...
      ####
      #       df.pred <- data.frame(x =df.ts$pseudotime,
      #                       y.pred =  predict( gams[[gene.name]]),
      #                       y.obs = df.ts$expression)
...
temporalModuleSignatures <- function(df, time = df$time, module.list){
  ...
  for (j in 1:length(module.list)){
    ...
    df.mod.signature <- merge(df.mod.signature, new.df, by = "time")
    df.mod.standarderror <- merge(df.mod.standarderror, new.df.se, by = "time")
...
n.cluster <- length(unique(as.character(so.query$seurat_clusters)))
...
for (i in 1:length(u.lineage)){
  ...
  if (cluster.method == "tree"){
    ...
    # hihisum = apply(bin.mat.1, 1, function(x) sum(x > 0.9)),
    # lolosum = apply(bin.mat.1, 1, function(x) sum(x < 0.9)),
    # minRate = apply(bin.mat.1, 1, function(x) min(diff(x))),
    # peakRate = apply(bin.mat.1, 1, function(x) max(diff(x))),
    # peak.time = apply(bin.mat.1, 1, function(x) which.min(abs(x - max(x, na.rm = T)))),
    ...
    # signal.characteristics <-  apply(signal.characteristics, 2, function(x) x * (sd(x)/(mean(x))))
    # signal.median = apply(bin.mat.1, 1, median, na.rm = T),
    # trough.time = apply(bin.mat.1, 1, function(x)  which.min(abs(x - min(x, na.rm = T)))),
    sc.mat <- as.matrix(signal.characteristics)
    
    # 
    ce<-clusterMany(t(sc.mat), clusterFunction="pam", ks=2:n.cluster, minSizes=3, isCount = F,nFilterDims=c(100,500,1000),
                    reduceMethod=c("PCA"),nReducedDims=c(3, 10),run=TRUE)
    
    # ce<-clusterMany(t(bin.mat.1), clusterFunction="hierarchicalK", ks=2:n.cluster, minSizes=3, isCount = F,nFilterDims=c(100,500,1000),
    # reduceMethod=c("PCA"),nReducedDims=c(3, 10),run=TRUE)
    
    # ce<-clusterMany(t(bin.mat.1), clusterFunction="hierarchical01", alpha = 0.1, beta = 0.8, 
    #                 ks=2:n.cluster, minSizes=3, isCount = F,nFilterDims=c(100,500,1000),
    #                 reduceMethod=c("PCA"),nReducedDims=c(3, 10),run=TRUE)
  ...
  module.colours[which.unassigned] <- "ambigious"
  ...
  # peak.activity$ranking.var <- peak.activity$peak.time * peak.activity$half.maxauc
  ...
  df.mod.signature.long <- pivot_longer(df.mod.signature.collapsed, cols = names(module.list.collapse))
  ...
  df.mod.se.long <- pivot_longer(df.mod.se.collapsed, cols = names(module.list.collapse))
  ...
  plt.sig <- ggplot() +
    ...
    geom_ribbon(data = df.mod.signature.long, aes(x = time, y = value, ymin = value-ci, ymax = value+ci), 
...
for (i in 1:length(all.lineages)){
  ...
  plt.hm <- pheatmap::pheatmap(ph.mat.sub.2,
                               ...
                               main = all.lineages[i],
                               gaps_row = gap.ind,
                               annotation_colors = list(Pseudotime = viridis(n = 20, option = "D"), module = u.color), 
                               show_colnames = F,
                               show_rownames = F)
...
```{r topGO enrichment, fig.height=8, fig.width=7, warning = FALSE}


# original.lim <-  memory.limit()
# new.lim <- 25000
# memory.size(new.lim)
...
enrich.results <- foreach(i = 1:length(module.membership.list), .packages = c("dplyr", "ggplot2", "scMiko")) %dopar% {
  ...
  # store results
  enrich.results[[lineage.name]] <- list(
    plot = plt.enrich,
...
names(enrich.results) <- names(module.membership.list)
...
for (i in 1:length(all.lineages)){
  ...
  lineage.name <- all.lineages[i]
  # if (!grepl("scPedigree", lineage.name)) next
  # lineage.name.short <- gsub("scPedigree.", "", lineage.name)
...
```{r gi cor mm, fig.height=5, fig.width=6}

pheatmap::pheatmap(cor.res, main = "Gene Importance Correlations")
```

### Gene Importance Correlations (Method x Gene)
```{r gi cor mg, fig.height=4, fig.width=22}
...
```{r ensemble meta-analysis, fig.width=12,fig.height=4}
