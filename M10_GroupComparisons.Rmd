...
if (length(conserved.markers.list)>0){
  ...
  for (i in 1:length(conserved.markers.list)){
    ...
    x.var.name <- paste(reference.group, "_avg_logFC", sep = "")
    y.var.name <- paste(comparison.group, "_avg_logFC", sep = "")
    ...
    conserved.genes[[which.cluster]] <- cur.genes$gene[cur.genes$gene.type == 1]
...
df.umap <- data.frame(so.query.2@reductions[["umap"]]@cell.embeddings)
colnames(df.umap) <- c("x", "y")
df.umap$cluster <- so.query.2@meta.data[["seurat_clusters"]]
df.umap$group <- so.query.2@meta.data[["group.query"]]
...
for (i in 1:length(plt.cmarkers.scatter)){
  ...
  if (is.null(plt.right)){
    plt.cc.nc <- cowplot::plot_grid(plt.cur.cluster.umap, plt.cmarkers.scatter[[i]], ncol = 2, align = "hv")
  } else {
    plt.cc.nc <- cowplot::plot_grid(plt.cur.cluster.umap, plt.cmarkers.scatter[[i]], plt.right, ncol = 3, align = "hv")
  }
...
which.rep <- names(pct.rep)[pct.rep > 0.1]
...
sft <- getSoftThreshold2(s.mat, power =c(seq(1,20, by = 1)), 
                         network.type = which.network, rescale.adjacency = F, n.cores = 4, nBreaks = 16)
...
which.metanet <- "consensus" # "consensus" or "differential"
...
difNetworkGraph <- function(w.mat, w.1, w.2, module.genes, z.all, z.sub, connectivity.quantile = 0.9){
  ...
  plt.graph <- g.mat %>%
    ...
    geom_edges(aes(color = delta.link, size =2*weight/max(weight, na.rm = T), alpha = weight/max(weight, na.rm = T))) + 
    ...
    new_scale("color") +
    geom_nodes(aes(fill = (node.type), size = 10*wi/max(wi, na.rm = T)), shape = 21, color = "grey60") +
...
```{r dash all, fig.height = 8, fig.width=18, warning = FALSE, message=FALSE}
...
if (!exists("ag.mat")){
  ag.mat <- aggGroupExpression(
    so.query.3,
    which.data = "data",  which.assay = DefaultAssay(so.query.3),
...
# get umap #####################################################################
df.umap <- data.frame(so.query.3@reductions[["umap"]]@cell.embeddings)
colnames(df.umap) <- c("x", "y")
...
for (i in 1:length(module.names)){
  ...
  results.co.p.cur <- results.co.p.cur[ ,c("pathway", "pval", "padj", "set")]
  ...
  plt.module.go <- results.co.p.cur %>%
    ...
    xlab("-log(p)") + labs(title = "Module Enrichment", subtitle = "GO Ontology", 
                           fill = "-log(p)", caption = "bar: p, dot: p adjusted.\nblack line: -log(0.05), grey line: -log(0.20).")
  ...
  plt.me.dif <- ggplot(df.ma.sub, aes(x = cluster.orig, y = value, fill = cluster.group))+
    ...
    labs(title = "EigenModule Activity", subtitle = "Stratified by clusters", fill = "Group",
  ...
  plt.mod.gene.dif <- z.mat.sub %>%
    ...
    geom_segment(aes(xend = 0, yend  = reorder(genes, dif))) + 
  ...
  try({
  pltNet <- difNetworkGraph(w.co, sn.1, sn.2, module.genes = module.genes, z.all = z.mat.all, z.sub = z.mat.sub, connectivity.quantile = 0.8)
  pltNet <- pltNet +theme(legend.position="bottom", legend.box = "vertical")    
  is.success <- T
  }, silent = T)
...
# df.con.deg$logp <- 
plt.3 <- df.con.deg %>%
  ggplot(aes(x = avg_logFC, y = rank(wi) ,color =avg_logFC, size = -log1p(p_val_adj) , text = genes)) +
  geom_point(alpha = 0.5) +
...
Cluster vs. rest-of-population differential expression analyses. 
