...
# ------------------------------------------------------------------------------
# input.file 
#     Seurat object path (i.e., file location)
# ------------------------------------------------------------------------------
# feature.format 
#     format of features provided by user. One of:
#                  0: user-provided genes. 
...
#     If feature.format = 3, 'features' argument is ignored.
# ------------------------------------------------------------------------------
# features.csv.path
#     Path to spreadsheet containing features. Required if using feature.format = 1. For intended use, features in spreadsheet must be organized in columns that can be referenced by the column header using the 'features' argument.
# ------------------------------------------------------------------------------
# discovery.method 
#     Differential expression algorithm used in discovery mode (feature.format = 2). One of:
#                    "Wilcoxon": Identifies DEGs using Wilcoxon Rank Sum test
#                    "CDI": Identifies DEGs using co-dependency test.
#                    "Gini": Identifies group-specific features using gini inequality index. 
# ------------------------------------------------------------------------------
# discovery.pct.min 
#     Features that are expressed below this value are omitted from differential expression analysis.  Range between [0,1], 0.1 is recommended. Required only if feature.format = 2. 
# ------------------------------------------------------------------------------
# discovery.group.by 
#     Name of object meta data field to group cells by when performing differential expression. E.g., "seurat_clusters". Required only if feature.format = 2. 
# ------------------------------------------------------------------------------
# as.module 
#     TRUE/FALSE (logical), specify whether features should be aggregated into single meta-program. 
# ------------------------------------------------------------------------------
# general.cluster.resolution 
#     Value of the resolution parameter, use a value above (below) 1.0 if you want to obtain a larger (smaller) number of communities.
# ------------------------------------------------------------------------------
# general.subsample.factor 
#     Value between [0,1] specifying fraction of cells to subsample. Use 1 for no subsampling. 
# ------------------------------------------------------------------------------
# general.clean.clusters 
#     TRUE/FALSE (logical), omit cells exceeding 3 median absolute deviations from cluster center in UMAP coordinate space. 
# ------------------------------------------------------------------------------
# general.save.pdf 
#     TRUE/FALSE (logical), save analysis figures as PDFs. 
# ------------------------------------------------------------------------------
# general.print.inline 
#     TRUE/FALSE (logical), print figures directly in R notebook. 
# ------------------------------------------------------------------------------
# general.barcode.recode 
#     Named list where names are new labels, and entires are old labels. If is provided for old labels, treated as wild card. e.g. list(in.vitro = c("invitro), in.vivo = NA). See scMiko::recordBarcode(...) for details. 
# ------------------------------------------------------------------------------
# cor.method
#     feature association method. One of:
...
# ------------------------------------------------------------------------------
# cor.top.n 
#     Number of top associated features to show. 70 is recommended. 
# ------------------------------------------------------------------------------
# cor.which.data
#     slot to use for feature association analysis. "data" is recommended. 
# ------------------------------------------------------------------------------
# cor.min.expr 
#     Features that are expressed below this value are omitted from feature association analysis. Range between [0,1], 0.025-0.1 is recommended.
# ------------------------------------------------------------------------------
...
parameter.list <- list(
  ...
  # input.file = "C:/Users/Owner/Dropbox/PDF Projects - JM/Data/scRNA-seq/01_sci-RNA-seq3_Hong_Kevin_Jason/NM_HH/Mesoderm/Papers/Pijuan-Sala 2019 Dataset/so_pijuan-sala2019.rds",
  features = c("CD34"),
  feature.format = 2, # 0 user-provided, 1 M09_type1_queries.csv, 2 discovery
  feature.csv.path = "M09_features.csv", # csv spreadsheet containing features of interest, called by column name. Required only if feature.format = 1 
  discovery.method = "Wilcoxon", # options: Wilcoxon, CDI, Gini
  discovery.pct.min = 0.1, # minimum expression fraction per group. 
  ...
  cor.top.n = 100,
  cor.which.data = "data",
  cor.min.expr = 0.1,               # min expressing fraction to be included in correlation
...
if (parameter.list$feature.format == 2){
  ...
  if (toupper(parameter.list$discovery.method) == "WILCOXON"){
    ...
    deg.gene <- findCDIMarkers(object = so.query, features.x = grouping_var, features.y = exprGene, ncell.subset = 5000, verbose = T)   
    deg.gene.top <- deg.gene %>%
...
plt.sgExp.all <- pbapply::pblapply(1:length(available_markers), function(x){
  ...
  plt.sgExp.clust <- expression.Plot(so = so.query, e.mat = e.mat.log, f.mat = f.mat.clust, 
    ...
    # labs(title = "Violin Plots", subtitle = "Clusters", 
...
if (compute.cor) {
  ...
  # co-dependency index ########################################################
    ...
    cdi.df.all <- findCDIMarkers(object = so.query, features.x = available_markers, features.y = exprGene, verbose = T)
...
**Description**: Uniform manifold projection and approximation (UMAP) used to dimensionally reduce and visualize data while maintaining global transcriptomic structure. Clusters were determined using unsupervised Louvain community detection. 
...
Feature expression visualized using dot plot and heatmap.\

All plots have been hierarchically-clustered.\

Data are cluster-level expression averages, scaled by feature (column-wise).\

*Note* that if only one query features has been provided, the heatmap is not generated. 
...
**Description**: Feature summary reports
...
The **feature association table** displays the top features associated with the query features.\

The **similarity matrix** displays the hierarchically-clustered similarity between query- (x-axis) and associated- (y-axis) features. *Note* that if only one query feature was provided, a rank plot showing the top associated features is shown instead.\

In both reports, only the top N associated features are shown, and this may not reflect the entire list of associated features. 

The **significant features table** contains all features that were significantly associated with the query feature(s) (FDR < 0.001). 
