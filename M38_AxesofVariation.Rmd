...
parameter.list <- list(
  ...
  # input.file = "p12_cd34pos_only_220621.rds", 
...
prep.list <- prepSeurat2(so, e2s = gNames.list, 
                         ...
                         subset.data = parameter.list$subset.data, use.integrated = T,
...
if (gsea.n.workers > 1){
  ...
  gsea.res$plt.gsea <- gsea.res$plt.gsea 
...
gsea.plots <- pbapply::pblapply(gsea.res, function(x) x$plt.gsea + theme(axis.text.y = element_text(size=7)))
