...
# # get symbol to entrez conversion list
# all_enz_names <- as.list(org.Hs.egALIAS2EG)
# 
# # get chromsomal location for each gene
# chromosome_locations <- org.Hs.egCHRLOC
# 
# # Get the entrez gene identifiers that are mapped to chromosome locations
# mapped_genes <- mappedkeys(chromosome_locations)
# 
# # convert to list
# chromosome_location_entrez.list <- as.list(chromosome_locations[mapped_genes])
# 
# 
# # symbol to entrez mapping
# 
# # my.symbol <- as.vector(as.vector(gNames.list))
# 
# my.symbol <- names(all_enz_names)
# # my.entrez <- unlist(all_enz_names)
# my.entrez <- lapply(all_enz_names, function(x) unlist(x)[1])
# 
# 
# # my.entrez <- sym2entrez(my.symbol, my.species = which.species )
# my.entrez <- my.entrez[complete.cases(my.entrez), ]
# 
# e2g <- my.entrez$SYMBOL
# names(e2g) <- my.entrez$ENTREZID
# 
# chromosome_location_symbols.list <- chromosome_location_entrez.list
# names(chromosome_location_symbols.list) <- e2g[names(chromosome_location_symbols.list)]
...
e.mat <- as.data.frame(so.query@assays[["SCT"]]@data)
# e.mat <- e.mat[1:15]

ic.data <- infercnv::data
ic.genes <- infercnv::genes
ic.annot <- infercnv::annots

so.data <- e.mat
so.genes <- infercnv::genes
so.annot <- data.frame(V2 = rep("tumor", ncol(e.mat)))
rownames(so.annot) <- colnames(e.mat)

infercnv_obj <- infercnv::CreateInfercnvObject(raw_counts_matrix=so.data, 
...
# subsample_n <- 100
# 
# df.ma_sub <- df.ma[ , 1:subsample_n]
# 
# 
# df.ma_long <- df.ma_sub %>%
#   rownames_to_column() %>%
#   gather(cellID, value, -rowname)

# plt.chr_heatmap <- ggplot(df.ma_long, aes(x = rowname, y = cellID, fill = value)) +
#   geom_tile() +   scale_colour_gradient2()
# 
# print(plt.chr_heatmap)

# 
# 
# df.ma_asmat_trans <- t(as.matrix(df.ma_sub))
# 
# 
# gplots::heatmap.2(df.ma_asmat_trans, trace = "none")
