...
if (toupper(parameter.list$integration.method) == "BBKNN" ){
  ...
  so <- GetResidual(object = so,features =  so.features, assay = "SCT", replace.value = T)
  
  av.feat <- so.features[so.features %in% rownames(so@assays[["SCT"]]@scale.data)]
  
  miko_message("Running PCA...")
  so <- RunPCA(so, features = av.feat, assay = "SCT")
  ...
  so@assays[["integrated"]]@scale.data <- matrix()
  so@assays[["integrated"]]@counts <- as.sparse( matrix())
  so@assays[["integrated"]]@data <- as.sparse( matrix())
  DefaultAssay(so) <- "integrated"
  so <- RunUMAP(so, graph = "integrated_snn")
